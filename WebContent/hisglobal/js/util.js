...
function ajaxFunction(myurl,mode)
{
	...
	userMode = mode;
	myurl=replaceAll(myurl,'^','___');
	myurl=replaceAll(myurl,'|','a__b__c');
...
function adt_create_loading_msg()
{
	...
	//document.getElementById("normalMsg").innerHTML='<div id="qmvi_loading_div" style="top:'+tpos+'px;left:'+lpos+'px;height:'+qh+'px;width:'+qw+'px;position:absolute;font-size:70px"><i class="fas fa-spinner text-danger"></i></div>';
	//document.getElementById("normalMsg").style.display="";
	//$("#qmvi_loading_div").modal();
}
/***end***/
...
function sendReq()
{
    ...
    if (objXmlHttp.readyState==4 || objXmlHttp.readyState==200) 
    {
		...
		// getAjaxResponse(res,userMode);
	     ...
	     //var obj=document.getElementsByClassName("modal-backdrop show");
	     //console.log(obj);
	     //obj[0].className="";
	     try
	     {
...
function openDetailedAccInfo ()
{
	
	if(document.getElementById("accSideListId").style.display=="none")
	{
		document.getElementById("accSideListId").style.display="block";
		document.getElementById("accSideListId").focus();
		
	}
	else
		document.getElementById("accSideListId").style.display="none";
}
function showSlip()
{
...
{
	document.forms[0].strPayMode.selectedIndex=0;
}
/*function checkMsg(errID,normalID,wrnID) 
{
...
//	var warn=document.getElementById("wrnID");
	...
	var err=document.getElementById(errID);
	var nor=document.getElementById(normalID);
	var warn=document.getElementById(wrnID);
	if (err.innerHTML != "") {
...
function openPopupWithEventHeightWidth(url,eventObj, height, width,crfield,goid)
{
	if(document.getElementById("searchPopupModalID")==undefined)// Append Modal Only When it is not present on document
	{
		document.body.innerHTML +="<div class='modal' id='searchPopupModalID'><div class='modal-dialog modal-lg' style='max-width:90vw; max-height: 65vh;'><div class='modal-content'><div class='modal-header' style='height:11vh;'><h3 class='modal-title text-left'>Patient Search</h3><button type='button' class='close' data-dismiss='modal' style='margin-top:-36px;'>×</button></div><div class='modal-body' style='width:90vw; height: 65vh;'><iframe src='' style='width: 100%; height: 100%; border: none;'></iframe></div></div></div></div>";
	}
	   
	
	var gt=window.parent.document.getElementsByName("varSSOTicketGrantingTicket")[0].value;
	var targetURL= url+"&crFieldName="+crfield+"&goBtnId="+goid+"&varSSOTicketGrantingTicket="+ gt;
	
	$("#searchPopupModalID").find('iframe').attr('src',targetURL);
    //$("#searchPopupButtonID").trigger('click');
	//$("#searchPopupModalBodyId").load(url);

...
function validate(){
	...
	var hisValidator = new HISValidator("saleRptBean");
	...
	if(retVal){
		...
		document.forms[0].submit();  */
	 	...
	 	var hmode = "SHOWRPT"; 
		var url = "SaleReportCategoryCNT.cnt?hmode="+hmode+
		...
		"&patientType="+document.forms[0].strPatientCategory.value+
		"&patientTypeName="+document.forms[0].strPatientCategory[document.forms[0].strPatientCategory.selectedIndex].text+
...
function getPatientCatCmb(){ 

		if(document.forms[0].strStoreId.value!=0){
			var url ="SaleReportCategoryCNT.cnt?hmode=ITEMCATCMB&storeId="+document.forms[0].strStoreId.value;
	 		ajaxFunction(url,"1");
	 	}else{
...
function getAjaxResponse(res,mode){
	...
	if(mode=="1"){ 
			...
			objVal.innerHTML = "<select name ='strPatientCategory' class='custom-select' onChange=' ' >"+res+"</select>";		
	}	
	...
	if (mode == "7") {
		...
		document.getElementsByName("strPatientCategory")[0].disabled = true; 
...
<html:form action="/reports/SaleReportCategoryCNT" method="post">
	
<div class="errMsg" id="errMsg"><bean:write name="saleRptBean" property="strErrMsg"/></div>
<div class="normalMsg" id="normalMsg"><bean:write name="saleRptBean" property="strNormalMsg"/></div>
<div class="warningMsg" id="warningMsg"><bean:write name="saleRptBean" property="strWarningMsg"/></div>
	...
	<div class="row ">
		...
		<div class="container">
			...
			<div class="row" style="margin: 1% 12%;">
				...
				<div class="col-sm-4">
					<select name="strStoreId" class='custom-select' onchange="getPatientCatCmb();">  
					<!-- <select name="strStoreId" class='custom-select' onchange="">  -->
						<bean:write name="saleRptBean" property="strStoreValues" filter="false" />
					</select>
				</div>
		
				
				<div class="col-sm-2 "><label>Patient Category<font color="red">*</font></label></div>
				<div  class="col-sm-4" id="patientCatDivId">
					<select	name="strPatientCategory"  class="custom-select">
						<bean:write name="saleRptBean" property="strPatCatVal" filter="false" />
					</select>
					<!-- 
					<select	name="strPatientCategory"  class="custom-select">
						<option value="0">Select Value</option>
						<option value="11">General</option>
						<option value="27">EHS</option>					
						<option value="12">BPL</option>					
						<option value="86">Ayush</option>	
						<option value="87">Ayush General</option>	
					</select> -->
				</div> 
			</div>
		...
		<input type="hidden" name="strCurrentDate" value="${saleRptBean.strCurrentDate}"/>

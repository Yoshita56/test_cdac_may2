...
<script type="text/javascript">
		...
		$('#strSearchDrug').autocomplete(
				{
					...
					onSelect : function(suggestion) {
						...
						getDrugNameSelectedInLeftBox(suggestion.data,
								"strLeftItemIds");
					}
				});
	...
	function validate1() {
		...
		if (len > 0) {
			...
			for (var i = 0; i < len; i++) {
				if (i == 0) {
					...
					strTmpItemBrandId = strTmpItemBrandId + ","
							+ document.forms[0].strRightItemIds[i].value;
				}
			}
		...
		var hisValidator = new HISValidator("stockOnHandIgimsRpt");
		...
		if (retVal) {
			...
			//var strUserValue= document.forms[0].strUserValue.value;
	...
	function getItemCatCmb() {
		...
		if (document.forms[0].strStoreId.value != 0) {
			...
			var url = "StockOnHandIgimsRptCNT.cnt?hmode=ITEMCATCMB&storeId="
	...
	function getGroupCmb() {
		...
		if (document.forms[0].strItemCatNo.value != 0) {
			var url = "StockOnHandIgimsRptCNT.cnt?hmode=GROUPCMB&itemcat="
	...
	function getDrugName1() {
		...
		var url = "StockOnHandIgimsRptCNT_NEW.cnt?hmode=DRUGNAME&storeId="
				+ document.forms[0].strStoreId.value + "&itemcat=0"
	...
	function getDrugName() {
		...
		// 		 value="10">Active
		// 		 value="9">Re-Order Level
		if (document.forms[0].strStatusId.value != "9") {
			var url = "StockOnHandIgimsRptCNT.cnt?hmode=DRUGNAME&storeId="
					+ document.forms[0].strStoreId.value + "&itemcat="
					+ document.forms[0].strItemCatNo.value + "&groupId="
					+ document.forms[0].strGroupId.value + "&itemType="
					+ document.forms[0].strItemType.value + "&statusId="
					+ document.forms[0].strStatusId.value + "&districtStoreId="
					+ districtPresent;
			ajaxFunction(url, "7");
			shiftAllToLeft('strLeftItemIds', 'strRightItemIds', 1);
		}
	...
	function getDrugName3() {
		...
		var url = "StockOnHandIgimsRptCNT.cnt?hmode=DRUGNAME1&storeId="
				+ document.forms[0].strStoreId.value + "&itemcat="
				+ document.forms[0].strItemCatNo.value + "&groupId="
				+ document.forms[0].strGroupId.value + "&itemType="
				+ document.forms[0].strItemType.value + "&statusId="
	...
	function getStoreTypeCmb() {
		...
		//alert("in");
		var url = "StockOnHandIgimsRptCNT.cnt?hmode=STORETYPECMB&strDistrictStoreId="
				+ strDistrictStoreId;
		ajaxFunction(url, "4");
	...
	function getSubStoreCmb() {
		...
		var url = "StockOnHandIgimsRptCNT.cnt?hmode=SUBSTORECOMBO&strDistrictId="
				+ strDistrictId
				+ "&strCircleId="
				+ strCircleId
				+ "&strDistrictStoreId="
	...
	function getAjaxResponse(res, mode) {
		...
		if (mode == "1") {
			...
			objVal.innerHTML = "<select name ='strItemCatNo' class='browser-default custom-select' onchange='getGroupCmb();'>"
					+ res + "</select>";
			getDrugName();
		}
		...
		if (mode == "2") {
			...
			objVal.innerHTML = "<select name ='strDistrictId' class='browser-default custom-select' onchange='getStoreCmb();'>"
					+ res + "</select>";
		}
		...
		if (mode == "4") {
			...
			objVal.innerHTML = "<select name ='strStoreTypeId' class='browser-default custom-select' onchange='getSubStoreCmb();'>"
					+ res + "</select>";
			//getProgName();
		...
		if (mode == "5") {
			...
			objVal.innerHTML = "<select name ='strStoreId' class='browser-default custom-select' onchange='getDrugName1();'>"
					+ res + "</select>";
		}

		if (mode == "6") {
			var objVal = document.getElementById("groupDivId");
			var temp = "<select name ='strGroupId' class='browser-default custom-select' onchange='getDrugName3();'>"
					+ res.split("^")[0]
					+ "</select><input type='checkbox' name='strIsGroupWise' value='1' checked='false'>Batch Wise Report";

			var objVal1 = document.getElementById("itemDivId");
			var temp1 = "<select name ='strItemType' class='browser-default custom-select' onchange=' '>"
					+ res.split("^")[1]
					+ "</select><input type='hidden' name='strIsItemWise' value='1' >";
		...
		if (mode == "7") {
			...
			objVal.innerHTML = "<select id='strLeftItemIds' name='strLeftItemIds' size='6' class='form-control' multiple style='max-height:120px' onChange='showSelection(this);' >"
					+ res.split("^")[0] + "</select>";

			var objVal1 = document.getElementById("drugNameDivId");
			objVal1.innerHTML = "<select id='strDrugName' name='strDrugName' class='browser-default custom-select'  >"
					+ res.split("^")[0] + "</select>";
			loadAutocompleteItems();
		...
		if (mode == "8") {
			...
			objVal1.innerHTML = "<select id='strProgId' name='strProgId' class='browser-default custom-select'  >"
					+ res + "</select>";
			getDrugName1();
		}
		if (mode == "9") {
			...
			var temp1 = "<select name ='strItemType' class='browser-default custom-select' onchange='getDrugName4();'>"
					+ res.split("^")[2]
					+ "</select><input type='hidden' name='strIsItemWise' value='1' >";
	...
	function checkWhetherConsolidatedStockVisibility() {
		...
		if (userLevel == "1" || userLevel == "2" || userLevel == "3") {
			if (selectedCircle == "All" && selectedDistrict == "All"
					&& selectedStoreType == "13") {
...
<body class="background" onload="getDrugName();">
	...
	<html:form action="/reports/StockOnHandIgimsRptCNT" method="post" styleClass="formbg">
		<div class="errMsg" id="errMsg">
			<bean:write name="stockOnHandIgimsRpt" property="strErrMsg" />
		</div>
		<div class="normalMsg" id="normalMsg">
			<bean:write name="stockOnHandIgimsRpt" property="strNormalMsg" />
		</div>
		<div class="warningMsg" id="warningMsg">
			<bean:write name="stockOnHandIgimsRpt" property="strWarningMsg" />
		</div>
			...
			<div class="row">
				<div class="prescriptionTile col-sm-12" align="center">
						...
						<div class="col-sm-3">
							<div id="strSubStoreDivId">
								<select name="strStoreId" class="browser-default custom-select"
									...
									<bean:write name="stockOnHandIgimsRpt"
						...
						<div class="col-sm-3" align="left">
							<div id="itemCatDivId">
								<select name="strItemCatNo"
									class="browser-default custom-select" onchange="getDrugName();">
									<bean:write name="stockOnHandIgimsRpt"
										property="strItemCatgCombo" filter="false" />
									<!--  
					...
					<div class="row">
						...
						<div class="col-sm-3">
							<div id="groupDivId">
								<select name="strGroupId" class="browser-default custom-select"
									onchange=' '>
									<bean:write name="stockOnHandIgimsRpt" property="strGroupCombo"
										filter="false" />
									<option value="0">All</option>
								</select> <input type='checkbox' name='strIsGroupWise' value='1'
									checkecd='false'>Batch Wise Report
							</div>
						</div>
						...
						<div class="col-sm-3" align="left">
							<div id="itemDivId">
								<select name="strItemType" class="browser-default custom-select"
									onchange=" ">
									<bean:write name="stockOnHandIgimsRpt"
										property="strItemTypeValues" filter="false" />
									<option value="0">All</option>
								</select> <input type='hidden' name='strIsItemWise' value='1'>
							</div>
						</div>
					...
					<div class="row">
						...
						<div class="col-sm-3">
							<div id="drugNameDivId" style="display: none;">
								<select id="strDrugName" name="strDrugName"
									class="browser-default custom-select">
									<bean:write name="stockOnHandIgimsRpt" property="strDrugCombo"
										filter="false" />
								</select>
							</div>
							<div id="DrugNameId" style="display: none;"></div>

							<input type="text" id="strSearchDrug" class="form-control"
								name="strSearchDrug" size="80%" />
						</div>
						...
						<div class="col-sm-3">
							<div id="StatusDivId">
								<select name="strStatusId" class="browser-default custom-select"
									onchange="getDrugName();">
									<!-- <option title="All" selected="selected" value="0">All</option> -->
									<option title="Active" value="10">Active</option>
									<option title="Zero" value="12">Zero Stock</option> 
								</select>
							</div>
						</div>
					</div>
					...
					<div class="line">
						<table class="NEWTABLEWIDTH" cellspacing="1px" cellpadding="1px"
							align="center">
							<tbody>
								<tr>
					...
					<div class="row">
						<div class="col-sm-5">
							<div id="LeftItemIds" align="right">
								<select id="strLeftItemIds" name="strLeftItemIds"
									...
									<bean:write name="stockOnHandIgimsRpt"
										property="strLeftItemList" filter="false" />
								</select>
							</div>
						</div>
						...
						<div class="col-sm-2">
							<br>
							<center>
								<img src="../../hisglobal/images/forward3.gif" width="30"
						...
						<table class="TABLEWIDTH" align="center" cellspacing="1px"
							...
							<tr>
								...
								<td class="CONTROL" width="50%" colspan="2">
									<dateTag:date name="strDate" value="${stockOnHandIgimsRpt.strCurrentDate}"/>
								</td>
							</tr>
						</table>
					</div>
					...
					<table class="TABLEWIDTH" align="center" cellspacing="1px"
						...
						<tr>
							...
							<html:checkbox property="strIsFooter" name="stockOnHandIgimsRpt" value="1"></html:checkbox>
							</td>
						</tr>
					</table>
					<hr>
					...
					<input type="hidden" name="strCurrentDate" value="${stockOnHandIgimsRpt.strCurrentDate}" /> 
					<input type="hidden" name="strCurrentStock" value="${stockOnHandIgimsRpt.strCurrentStock}" /> 
					<input type="hidden" name="strSeatId" value="${stockOnHandIgimsRpt.strSeatId}" /> 
					...
					<input type="hidden" name="strIsDdwFlag" value="${stockOnHandIgimsRpt.strIsDdwFlag}" />
					<input type="hidden" name="strItemBrandId" value="${stockOnHandIgimsRpt.strItemBrandId}" />

					<!-- <input type="hidden" name="strUserValue" value=""/> -->
				</div>
			</div>
		</div>
	</html:form>

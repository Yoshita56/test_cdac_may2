...
function onClearPage()
{
	...
	document.getElementsByName("strTypeId")[0].disabled = false;
...
function getInventoryDtls() 
{
   	    ...
   	    var hisValidator = new HISValidator("JobDetailsRptbean");
		 
		hisValidator.addValidation("strCate", "dontselect=0", "Please Select Store Name" );
		 
		var retVal = hisValidator.validate();
		if(document.forms[0].strTypeId[document.forms[0].strTypeId.selectedIndex].value==0){
		...
		if (retVal) {
			...
			document.forms[0].strTypeName.value = document.forms[0].strTypeId[document.forms[0].strTypeId.selectedIndex].text;
			
			var hmode = "GOVIEWPAGE"; 
			var url = "JobDetailsRptCNT.cnt?hmode="+hmode+	
			"&strTypeId="+document.forms[0].strTypeId[document.forms[0].strTypeId.selectedIndex].value+
			"&strTypeName="+document.forms[0].strTypeName.value;
...
function getAjaxResponse(res,mode)
{
	...
	if (mode == "1") {
		...
		document.getElementById("strTypeId").disabled = true;
	}
}
...
<body>
	<html:form action="/reports/JobDetailsRptCNT"
		name="JobDetailsRptbean"
		type="mms.reports.controller.fb.JobDetailsRptFB" method="post">

		<div id="errMsg" class="errMsg">
			<bean:write name="JobDetailsRptbean" property="strErrMsg" />
		</div>
		<div id="warningMsg" class="warningMsg">
			<bean:write name="JobDetailsRptbean" property="strWarningMsg" />
		</div>
		<div id="normalMsg" class="normalMsg">
			<bean:write name="JobDetailsRptbean" property="strNormalMsg" />
		</div>
		<br>
		<br>
				...
				<div class="prescriptionTile col-sm-12" align="center">
					...
					<div class="container">
						<div class="row">
							...
							<div class="col-sm-3">
								<label class="float-right"><font color="red">*</font>Type</label>
							</div>
							<div class="col-sm-3">
									<select name="strTypeId"  id="strTypeId" class="browser-default custom-select">
										<option value="0">Select</option>
										<option value="11">Failure</option>
					...
					<input type="hidden" name="strTypeName" value="" />
					<input type="hidden" name="strCurrentDate" value="${JobDetailsRptbean.strCurrentDate}" />

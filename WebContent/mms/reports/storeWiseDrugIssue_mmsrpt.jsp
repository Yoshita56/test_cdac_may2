...
function validate(){
 		var hisValidator = new HISValidator("storeWiseDrugIssueRptBean");
		...
		hisValidator.addValidation("strToDate","dtgtet="+document.forms[0].strFromDate.value,"Please Select To Date Greater Than Or Equal To From Date");
            ...
            if(retVal){
            	var hmode = "SHOWRPT"; 
            	var url = "StoreWiseDrugIssueRptCNT.cnt?hmode="+hmode+
...
function getItemCateg(){
   ...
   var url="StoreWiseDrugIssueRptCNT.cnt?hmode="+mode+"&strId="+document.forms[0].strStoreId.value;
   ajaxFunction(url,"1");
}
...
function onClearPage()
{
	...
	document.getElementById("drugWiseDiv").style.display="none";
	document.getElementById("strStoreId").disabled = false; 
    document.getElementById("datepicker1").disabled = false;
    document.getElementById("datepicker2").disabled = false;
    document.getElementById("drugNameDivId").getElementsByTagName("select")[0].disabled = false;
    document.getElementById("itemCategDIV").getElementsByTagName("select")[0].disabled = false;
...
function getDrugName()
{
	...
	else
	{
		var mode="DRUGNAMECOMBO";
   		var url="StoreWiseDrugIssueRptCNT.cnt?hmode="+mode+"&catCode="+document.forms[0].strItemCategNo.value;
   		ajaxFunction(url,"2");
	}
...
function getBatchNo()
{
   ...
   var url="StoreWiseDrugIssueRptCNT.cnt?hmode="+mode
...
//	document.forms[0].strStoreId.disabled = true;
	if(obj.value==2)
	{
		...
		document.getElementById("drugWiseDiv").style.display="block";
	}
	else
	{
...
function printDataOne() {
	...
	newwin.document.write('<title>StoreWiseDrugIssue_Report</title>');
...
<html:form action="/reports/StoreWiseDrugIssueRptCNT" method="post">
	
<div class="errMsg" id="errMsg"><bean:write name="storeWiseDrugIssueRptBean" property="strErrMsg"/></div>
<div class="normalMsg" id="normalMsg"><bean:write name="storeWiseDrugIssueRptBean" property="strNormalMsg"/></div>
<div class="warningMsg" id="warningMsg"><bean:write name="storeWiseDrugIssueRptBean" property="strWarningMsg"/></div>
	
<div class="container-fluid">
	<div class="prescriptionTile">
		<div class='legendHeader' style='font-size: 16px;font-weight: bold;'>
		<i aria-hidden="true" class="fa fa-clipboard"></i> &nbsp;Store Wise Drug Issue Details
		</div>
		...
		<div class="row ">
			<div class="legend2" id='nonPrintableLegend2'>
				...
				<button type="button" title="Clear" class="float-right btn btn-secondary mt-1 btn-icon clearbtn" style="background-color: #2196f3; border-radius:50%; padding:12px 7px;" onClick="onClearPage();">	
					<div class="popupToast" style="color: #fff;">
		...
		<div class="container">
			<div class="row " style="margin: 1% 12%;">
				...
				<div class="col-sm-4">		
					<input type="radio" name="strRadioOne" value="1" onclick="radioChecked(this);"  checked> Store				
	     	  	    <input class="mx-1" type="radio" name="strRadioOne" value="2" onclick="radioChecked(this);"> Drug Wise
			...
			<div class="row " style="margin: 1% 12%;">
				...
				<div class="col-sm-4">
					<select name="strStoreId" id="strStoreId" class='custom-select' onchange="getItemCateg();"> 
						<bean:write name="storeWiseDrugIssueRptBean" property="strStoreVal" filter="false" />
					</select>
				</div>
				...
				<div class="col-sm-4" id="itemCategDIV">
					<select id="strItemCategNo" name="strItemCategNo"   class="custom-select" onchange="getDrugName();">
						<bean:write name="storeWiseDrugIssueRptBean" property="strItemCategCmb" filter="false" />
					</select>		
				</div> 
			</div>
			
			<div id="drugWiseDiv" style="display:none;">
				<div class="row " style="margin: 1% 12%; ">
			...
			<div class="row " style="margin: 1% 12%;">
				...
				<div class="col-sm-4">
					<input name="strFromDate" id="datepicker1" class="form-control datepicker1" value="${storeWiseDrugIssueRptBean.strCurrentDate}" />
				</div>
			
				<div class="col-sm-2"><label>To Date<font color="red">*</font> </label></div>
				<div class="col-sm-4">
					<input name="strToDate" id="datepicker2" class="form-control datepicker2" value="${storeWiseDrugIssueRptBean.strCurrentDate}" />
	 			</div>
			</div>	
...
<input type="hidden" name="strCurrentDate" value="${storeWiseDrugIssueRptBean.strCurrentDate}" />

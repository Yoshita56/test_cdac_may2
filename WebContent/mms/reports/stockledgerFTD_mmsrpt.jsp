...
<!-- EXT JS -->
<script language="Javascript" src="../../mms/js/stockledgerFTD_mmsrpt.js"></script>
...
<html:form action="/reports/FTDStockLedgerRptCNT" method="post" styleClass="formbg">
	
<div class="errMsg" id="errMsg"><bean:write name="stockLedgerBean" property="strErrMsg"/></div>
<div class="normalMsg" id="normalMsg"><bean:write name="stockLedgerBean" property="strNormalMsg"/></div>
<div class="warningMsg" id="warningMsg"><bean:write name="stockLedgerBean" property="strWarningMsg"/></div>
				...
				<div class="row my-2" id="itemManageDtlId1">
					...
					<div class="col-sm-2" id="storeDivId">
						<select name="strStoreId" id="storeDivId1" class='custom-select ' onchange="getItemCatCmb();"> 
							<bean:write name="stockLedgerBean" property="strStoreValues" filter="false" />
						</select>
					...
					<div class="col-sm-2" id="catDivId">
						<select name="strItemCatId"  class="custom-select " onchange='getGroupName();'>
							<bean:write name="stockLedgerBean" property="strCatCombo"	filter="false" />
					...
					<select name="strGroupId"  id="strGroupId1" class="custom-select " onchange='getDrugName();'>
						<bean:write name="stockLedgerBean" property="strGroupCombo"	filter="false" />
						<option value="0">Select Value</option>
					</select>					
					</div>		
				...
				<div id="itemManageDtlId2" style="display: block">	
						...
						<tr>
				 			...
				 			<td class="CONTROL" colspan="1" width="25%">  			 
								<div id="LeftItemIds" align="left">
									<select id="strLeftItemIds" name="strLeftItemIds" size="10" multiple style="width: 560px" onChange='showSelection(this);' >
										<bean:write name="stockLedgerBean" property="strLeftItemList" filter="false"/>
									</select>
								</div>				
							</td>
							...
							<div id="RightItemIds" align="left" style='display:inline;'>
									<select id="strRightItemIds" name="strRightItemIds" size="10" multiple style="width: 560px">
										<bean:write name="stockLedgerBean" property="strRightItemList" filter="false"/>
									</select>
								</div>
							</td>		
						</tr>		
					...
					<div class="row my-2">
						...
						<div class="col-sm-4 p-2">
							<html:checkbox property="strWhetherBatchWise" name="stockLedgerBean" value="1" >
							</html:checkbox>
						</div>
					
						<div class="col-sm-2"><label>From Date<font color="red">*</font></label></div>
						<div class="col-sm-4">
							<%-- <input name="strToDate" id="datepicker" class="form-control datepicker" value="${stockLedgerBean.strCurrentDate}" /> --%>	
							<input name="strFromDate" id="datepicker1" class="form-control datepicker1" value="${stockLedgerBean.strFYStartDate}" />
					...
					<div class="row my-2" style="">
						...
						<div class="col-sm-4 p-2">
							<%-- <html:checkbox property="strValueWise" name="stockLedgerBean" value="1" ></html:checkbox> --%>
						</div>
					
						<div class="col-sm-2">To Date</div>
						<div class="col-sm-4">
							<input name="strToDate" id="datepicker2" class="form-control datepicker2" value="${stockLedgerBean.strCurrentDate}" />
						</div>
					</div>
        			...
        			<hr>
				<div class="row" align="left"><i>Due to Bulky Movement of Data Please Don't Select More than 4 Drugs/Items in single shot !! </i></div>
				</div> <!-- Ending <div id=itemManageDtlId2 -->
	
				<input type="hidden" name="hmode"/>
				<input type="hidden" name="strCurrentDate" 		value="${stockLedgerBean.strCurrentDate}"/>
				<input type="hidden" name="strStoreName" 		value="${stockLedgerBean.strStoreName}" />
				<input type="hidden" name="strItemBrandId" 		value="0" />
				<input type="hidden" name="selPageIndex" 		value="1"/>
				<input type="hidden" name="strJobInitialDate" 	value="${stockLedgerBean.strJobInitialDate}"/>
				<input type="hidden" name="strFYStartDate" 		value="${stockLedgerBean.strFYStartDate}"/>
			</div>
			</fieldset>
		</div>
	</div>

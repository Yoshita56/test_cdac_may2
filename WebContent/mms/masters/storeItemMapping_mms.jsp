...
var len = parseInt(document.forms[0].strRightItemIds.length);
	...
	var hisValidator = new HISValidator("storeItemMappingBean");
...
//	hisValidator.addValidation("strItemType", "dontselect=0","Please Select Drug Type ");
	...
	hisValidator.clearAllValidations();
		...
		{			
	          ...
	          document.forms[0].strReOrder.value = document.forms[0].strReOrder.value;
	          document.forms[0].strMaxLevel.value = document.forms[0].strMaxLevel.value;
...
function getItemCatCmb(){ 
		...
		{
			var url ="StoreItemMappingMstCNT.cnt?hmode=ITEMCATCMB&storeId="+document.forms[0].strStoreId.value;
	 		ajaxFunction(url,"1");
	 	}
	 	else
	 	{
...
function getGroupCmb(){ 
	...
	{
		var url ="StoreItemMappingMstCNT.cnt?hmode=GROUPCMB&itemcat="+document.forms[0].strItemCatNo.value;
 		ajaxFunction(url,"6");
 	}
 	else
 	{
...
function getDrugName1()
{
	    ...
	    var url ="StoreItemMappingMstCNT_NEW.cnt?hmode=DRUGNAME&storeId="+document.forms[0].strStoreId.value+
		         "&itemcat=0"+
...
// 		 value="9">Re-Order Level
		...
		{	
			var url ="StoreItemMappingMstCNT.cnt?hmode=DRUGNAME&storeId="+document.forms[0].strStoreId.value+
...
function getDrugName3()
{
	    ...
	    var url ="StoreItemMappingMstCNT.cnt?hmode=DRUGNAME1&storeId="+document.forms[0].strStoreId.value+
		         "&itemcat="+document.forms[0].strItemCatNo.value+
		         "&groupId="+document.forms[0].strGroupId.value+
		         "&itemType="+document.forms[0].strItemType.value+
	   			 "&statusId="+document.forms[0].strStatusId.value+
...
function getStoreTypeCmb(){
		...
		//alert("in");
    	var url ="StoreItemMappingMstCNT.cnt?hmode=STORETYPECMB&strDistrictStoreId="+strDistrictStoreId;
    	ajaxFunction(url,"4");
...
var url ="StoreItemMappingMstCNT.cnt?hmode=SUBSTORECOMBO&strDistrictId="+strDistrictId+
...
/*
	...
	hisValidator.addValidation("strStoreId", "dontselect=0","Please Select Store Name");
	hisValidator.addValidation("strItemCatNo", "dontselect=0","Please Select Item Category");
 	hisValidator.addValidation("strGroupId", "dontselect=0","Please Select Group Type ");
 	hisValidator.addValidation("strItemType", "dontselect=0","Please Select Drug Type ");
...
*/
	...
	//            err.innerHTML="<i class='fas fa-exclamation-triangle'></i>"+"&nbsp;&nbsp;&nbsp;<strong>Error!</strong>&nbsp;&nbsp;"+err.innerHTML+"<button type='button' class='close' data-dismiss='alert'>&times;</button>";
	              err.innerHTML="<i class='fas fa-exclamation-triangle'></i>"+"&nbsp;&nbsp;&nbsp;<strong>Error!</strong>&nbsp;&nbsp;"+err.innerHTML;
	              err.style.display = "block";
	              
	      }
	      if (nor && nor.innerHTML != "") {
	              nor.innerHTML="<i class='far fa-check-circle'></i>"+"&nbsp;&nbsp;&nbsp;<strong>Success!</strong>&nbsp;&nbsp;"+nor.innerHTML;
	              nor.style.display = "block";
	      }
	      if (warn && warn.innerHTML != "") {
	              warn.innerHTML="<i class='fas fa-bell'></i>"+"&nbsp;&nbsp;&nbsp;<strong>Warning!</strong>&nbsp;&nbsp;"+warn.innerHTML;
...
<html:form action="/masters/StoreItemMappingMstCNT" name="storeItemMappingBean" type="mms.masters.controller.fb.StoreItemMappingMstFB" method="post" styleClass="formbg">
<br>

<center>
	<div id="errMsg" class="errMsg" style="font-size:16px"><bean:write name="storeItemMappingBean" property="strErrMsg" /></div>
	<div id="warningMsg" class="warningMsg" style="font-size:16px"><bean:write name="storeItemMappingBean" property="strWarningMsg" /></div>
	<div id="normalMsg" class="normalMsg" style="font-size:16px; display:block !important;"><bean:write name="storeItemMappingBean" property="strNormalMsg" /></div>
</center>
...
<div class="container">
				<div class="prescriptionTile col-sm-12" align="center">
					<div class="row rowFlex reFlex">
						...
						<div class="col-sm-6">
							<div class="legend2" id='nonPrintableLegend2'>
								...
								<button type="button" title="Clear" class="float-right btn btn-secondary mt-1 btn-icon " style="background-color: #2196f3; border-radius:50%; padding:8px 7px" onclick="onClickClear();">	
						...
						<div class="col-sm-3">
							<div id="strSubStoreDivId">
								<select name="strStoreId" class="browser-default custom-select"
									...
									<bean:write name="storeItemMappingBean" property="strStoreValues"
										filter="false" />
								</select>
							</div>
						</div>
						...
						<div class="col-sm-3" align="left" >
							<div id="itemCatDivId">
								<select name="strItemCatNo" class="browser-default custom-select"
									...
									<bean:write name="storeItemMappingBean" property="strItemCatgCombo"  filter="false" /> 
									<!--  
					...
					<div class="row">
						...
						<div class="col-sm-3">
							<div id="groupDivId">
								<select name="strGroupId" class="browser-default custom-select" onchange=' '>
									<bean:write name="storeItemMappingBean" property="strGroupCombo"
										filter="false" />
									<option value="0">All</option>
								</select> <input type='checkbox' name='strIsGroupWise' value='1' checkecd='false'>Batch Wise Report
							</div>
						</div>
						...
						<div class="col-sm-3" align="left" >
							<div id="itemDivId">
								<select name="strItemType" class="browser-default custom-select" onchange=" ">
									<bean:write name="storeItemMappingBean" property="strItemTypeValues"
										filter="false" />
									<option value="0">All</option>
								</select>
						...
						<div class="col-sm-3">
							<div id="drugNameDivId" style="display: none;">
								<select id="strDrugName" name="strDrugName" class="browser-default custom-select">
									<bean:write name="storeItemMappingBean" property="strDrugCombo" filter="false" />
								</select>
							</div>
						...
						<div class="col-sm-3">
							<div id="StatusDivId">
								<select name="strStatusId" class="browser-default custom-select"
									...
									<option title="Re-Order Level" value="9">Re-Order Level</option> 
						...
						<div class="col-sm-5">
							<div id="LeftItemIds" align="right">
								<select id="strLeftItemIds" name="strLeftItemIds"
									...
									<bean:write name="storeItemMappingBean" property="strLeftItemList"
										filter="false" />
								</select>
							</div>
						</div>
						...
						<div class="col-sm-5">
							<div id="RightItemIds" align="left">
								<select name="strRightItemIds" size="6" multiple=""
									...
									<bean:write name="storeItemMappingBean" property="strRightItemList"
										filter="false" />
								</select>
							</div>
						</div>
					...
					<div class="row">
						...
						<div class="col-sm-3">
							<input type="text" id="strReOrder" class="form-control" name="strReOrder" />
						</div>
						...
						<div class="col-sm-3">
							<input type="text" id="strMaxLevel" class="form-control" name="strMaxLevel" />
						</div>
					</div>
					...
					<div id="dateDivId" style="display: none;">
						<table class="TABLEWIDTH" align="center" cellspacing="1px" cellpadding="1px">
							<tr>
								...
								<td class="CONTROL" width="50%" colspan="2"><dateTag:date name="strDate" value="${storeItemMappingBean.strCurrentDate}" /></td>
							</tr>
						</table>
					</div>
					...
					<table class="TABLEWIDTH" align="center" cellspacing="1px" cellpadding="1px"  style="display: none;">
						<tr>
							...
							<td width="50%" colspan="1" class="CONTROL">
								<html:checkbox property="strIsFooter" name="storeItemMappingBean" value="1"></html:checkbox>
							</td>
						</tr>
					</table>
<hr>
					...
					<input type="hidden" name="strCurrentDate" value="${storeItemMappingBean.strCurrentDate}" />
					<input type="hidden" name="strCurrentStock" value="${storeItemMappingBean.strCurrentStock}" />
					<input type="hidden" name="strSeatId" value="${storeItemMappingBean.strSeatId}" />
					<input type="hidden" name="strStoreName" value=""/>
					<input type="hidden" name="strGroupName" value=""/>
					<input type="hidden" name="strMaxLevel" value=""/>
					<input type="hidden" name="strReOrder"  value=""/>
					<input type="hidden" name="strDrugTypeName"  value=""/>
					<input type="hidden" name="strIsDdwFlag" value="${storeItemMappingBean.strIsDdwFlag}" />
					<input type="hidden" name="strItemBrandId" value="${storeItemMappingBean.strItemBrandId}" />

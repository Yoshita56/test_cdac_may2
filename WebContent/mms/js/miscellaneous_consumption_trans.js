...
function getItemCategoryCmb()
{
	...
	url="MiscellaneousIgimsTransCNT.cnt?hmode="+mode+"&storeComboId="+document.forms[0].strStoreId.value;
...
function getGroupCombo()
{
	...
	url="MiscellaneousIgimsTransCNT.cnt?hmode="+mode+"&itemCatId="+document.forms[0].strItemCategoryId1.value; 
 	ajaxFunction(url,"2");
...
function getViewItemDtl() {
	...
	else if (document.getElementsByName("strItemCatgCombo")[0].value == "") 
	{
	...
	else 
	{
		...
		var mode = "GOVIEWPAGE";
		var url = "MiscellaneousIgimsTransCNT.cnt?hmode=" + mode + "&storeId="
...
function getIssueDtls(strMode, strStoreId, strIssueNo,strIndentNo,strIndentDate,ucReq) {
	...
	if (itemStockObj.innerHTML == "") {
		...
		var url = "MiscellaneousIgimsTransCNT.cnt?hmode=" + hmode + "&strMode=" + strMode
...
function validate1() {
   	...
   	var hisValidator = new HISValidator("miscellaneousTransIgimsBean"); 

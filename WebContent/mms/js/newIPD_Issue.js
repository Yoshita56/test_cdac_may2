...
function getItemDetails() {
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=ITEMDETAILS&reqNo=" + gblObject
...
function getConsultantCombo() {
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=PRESCRIPEDBY&unitCode="
...
function getUnitCombo() {
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=UNITCMB&deptCode="
...
function getRequestDetail(obj) {
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=REQUESTDTLS&reqNo=" + obj.value
...
function getPrevIssueDtl() {
	...
	var strItemCategory ="10";
	if(strFromStoreId !=0){
		var url = "NewIPDIssueToPatTransCNT.cnt?hmode=PREVISSUEDTL&strId="
...
function getItemCat() {
	...
	if (strStoreId != null && strStoreId != "0") {
		var url = "NewIPDIssueToPatTransCNT.cnt?hmode=ITEMCATCMB&storeId="
...
function getIssuePopUp(these, index) 
{
	...
	$(these).closest("div").find("i").addClass("fa-folder-open");
		...
		objGlobal = these;
		var url = "NewIPDIssueToPatTransCNT.cnt?hmode=ISSUEDTLPOPUP&strId="
	...
	if(mode=="11")
	{
		 		...
		 		//	{
		 			//	saveObj.style.display = "none"; 
		 				...
		 				var hisValidator = new HISValidator("newIPDIssueToPatBean");
...
{	
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=GET_PAT_ACC_STATUS&crNo="+document.forms[0].strCrNo.value+"&strId="+ document.forms[0].strStoreId.value.split("^")[0];
	if(mode=="1")
	{	
	    ajaxFunction(url, "19");
	}
...
function getViewItemDtl() 
{
	...
	else 
	{
		...
		var mode = "GETCRDTLS";
		var url = "NewIPDIssueToPatTransCNT.cnt?hmode=" + mode + "&storeId="
...
function validateTariff()
{
	...
	var itemParVal     = document.getElementsByName("itemParamValue");
	 ...
	 var url = "NewIPDIssueToPatTransCNT.cnt?hmode=TARIFFCHK&itemTmp="+ itemTmp;
	ajaxFunction(url, "11");
...
function getReport()
{
	...
	// alert("storeId"+storeId);
	 ...
	 { 
	   ...
	   var url="NewIPDIssueToPatTransCNT.cnt?hmode="+mode+"&strIssueNo="+ transferNo+"&strStoreId="+storeId+"&strMode=1";
	   ajaxFunction(url,"9");
	 }
}
...
function getReport2()
{
	 ...
	 { 
	   ...
	   var url="NewIPDIssueToPatTransCNT.cnt?hmode="+mode+"&issueNo="+ transferNo+"&storeId="+storeId;
	   ajaxFunction(url,"8");
	 }
...
function getReqType(){
	//alert('11');
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=GETREQTYPE&storeId=" + document.forms[0].strStoreId.value+"&itemCatId=" + document.forms[0].strItemCat.value+"&patStatus="+ document.forms[0].strPatStatus.value ;
	//alert(url);
	ajaxFunction(url,"12");
}

function getPresData(){
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode=GETPRESCDATA&storeId=" + document.forms[0].strStoreId.value+"&strCrNum=" + document.forms[0].strCrNo.value+"&deptID=" + document.forms[0].strDeptCode.value+"&strDeptUnit=" + document.forms[0].strUnitCode.value+"&episodeDtl=" + document.forms[0].strOffLineEpisode.value;
	//alert(url);
	ajaxFunction(url,"18");
}
...
function checkEpidoseData()
{
	...
	var itemTmp1 = itemTmp.substring(0, itemTmp.length - 1) ; 
	 ...
	 var url = "NewIPDIssueToPatTransCNT.cnt?hmode=ALREADYISSUEDRUG&itemTmp="+ itemTmp1+"&strCrNum="+document.forms[0].crNo.value+"&episodeDtl="+document.forms[0].strOffLineEpisode.value;
...
function getEpisodeList(obj){
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].crNo.value+"&deptCode="+obj.value+"&unit="+document.forms[0].strUnitCode.value;
	ajaxFunction(url,"16");
...
{    
	...
	{	  
		...
		{		   
		    ...
		    var url = "NewIPDIssueToPatTransCNT.cnt?hmode=" +hmode+ "&strMode=" +strMode+ "&strStoreId=" +strId+ "&strIssueNo=" + issueNo;
		    ajaxFunction(url, "8");
		}
	}
	document.forms[0].strIssueNum.value = "0";	
...
function getPresForm(){
	...
	var url = "NewIPDIssueToPatTransCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].crNo.value+"&episodeCode="+document.forms[0].strOffLineEpisode.value;
	ajaxFunction(url,"17");

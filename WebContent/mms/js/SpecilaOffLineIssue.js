...
{
	var hisValidator = new HISValidator("offLineIssueForSpPPBean");
...
{      		   
        ...
        var  taxObj         = document.getElementById("strReqStoreAvlQty"+index).value;
		var  qtyObj         = document.getElementById("strIssueQty"+index).value;
		var  qty_base_value = document.getElementById("strUnitName"+index).value.split("^")[1];
		
		if(isNaN(taxObj) || taxObj=="")
		{
			alert("Please Enter GST/Tax !!");
			return false;
		}
		...
		//alert("RATE_WITHOUT_TAX---"+rate);
				
		if(taxObj != "" && taxObj.length > 0)
	    {
			rate = parseFloat(rate) + (parseFloat(rate) *	parseFloat(taxObj))/100;
	    }
	    
		
		//alert("RATE_WITH_TAX---"+rate);
	   ...
	   if(count=='0')
  	   {
		  // alert(thisImg.title);
			...
			if (thisImg.title == "Show") 
			{
				...
				thisImg.class = "fas fa-minus-circle";
				
				document.getElementById("imgStockDetails").class="fas fa-minus-circle";
				thisImg.title = "Hide";
			...
			{  
				...
				thisImg.class = "fas fa-plus-circle";
				document.getElementById("imgStockDetails").class="fas fa-plus-circle";
				thisImg.title = "Show";
...
function getItemCategoryCombo() 
{
	...
	if(temp==null || temp=="" || temp=="0") {
		...
		objVal.innerHTML = "<select name = 'strItemCatgCombo' onchange='getIndentStoreCombo();' class='form-control'><option value='0'>Select Value</option></select>";
		return;
	}
	var mode = "ItemCatgoryCombo";
	var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + mode + "&modName="
			+ temp;

	ajaxFunction(url, "2");
	var objVal1 = document.getElementById("IndentingStore");
	objVal1.innerHTML = "<select name = 'strIndentingStoreID' class='form-control'><option value='0'>Select Value</option></select>";
...
function getPendingDemandDetails(index)
{
     ...
     {  
			if (document.forms[0].strIsView.value == '0') 
			{
				...
				var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + mode + "&modName="+hiddenVal+"&issueStoreName="+temp1+"&avlBudget="+document.forms[0].strAvalaibleBudget.value+"&budgetFlg="+document.forms[0].strBudgetFlg.value;
...
function getApprovedByCombo() 
{
	...
	if(temp==null || temp=="" || temp=="0") 
	{
		...
		objVal.innerHTML = "<select name = 'strApprovedBy'  class='form-control'><option value='0'>Select Value</option></select>";
		return;
	}
	var mode = "ApprovedByCombo";
	var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + mode + "&modName=" + temp;
    ajaxFunction(url, "7");
}
  ...
  {  
	if (document.forms[0].strIsView.value == '0') 
	{
	  ...
	  {	
		...
		var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + mode + "&modName="+ mainStore + "^" + itemCatg+"^"+indentingStore;
...
function getAjaxResponse(res, mode) {
	...
	if (mode == "1") {
		var objVal1 = document.getElementById("IndentingStore");
		objVal1.innerHTML = "<select name = 'strIndentingStoreID' onchange='getApprovedVerifyCombo();' class='form-control'>"
				+ res + "</select>";
	...
	if (mode == "2") 
	{
		...
		var objVal = document.getElementById("ItemCatg");
		objVal.innerHTML = "<select name = 'strItemCatgCombo' onchange='getIndentStoreCombo();' class='form-control'>"
				+ res + "</select>";
				if(document.forms[0].strIsView.value=='0')
	            {
				   getPendingDemand();
	...
	if (mode == "3") {
		...
		objVal.innerHTML = "<select name = 'strItemCatgCombo' onchange='getGroupNameCombo();' class='form-control'>"
				+ res
				+ "</select>"
	...
	if (mode == "6")
	{
		...
		document.getElementById("VerifiedBy").innerHTML = "<select name = 'strVerifiedBy' class='form-control'>"
				+ res + "</select>";
		document.getElementById("ReceivedBy").innerHTML = "<select name = 'strReceivedBy' class='form-control' onChange='OtherApprovedByfn();'>"
				+ res + "</select>";
	...
	{
		var objVal = document.getElementById("ApprovedBy");
		objVal.innerHTML = "<select name = 'strApprovedBy'  class='form-control'>"+ res + "</select>";
	}
	if (mode == "8") 
	{
...
function getViewItemDtl() {
	...
	if (document.getElementsByName("strStoreName")[0].value == "0") {
		...
		var mode = "GOVIEWPAGE";
		var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + mode + "&storeId="
				+ temp + "&itemCatNo="
...
function showPopUp(obj, i) 
{
	...
	var mode = "GETPOPUP";
	var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + mode + "&hiddenVal="
...
function getReport() 
{
	...
	if (issueNo != "0") 
	{
		...
		var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + hmode + "&strMode=2&strStoreId=" + storeId + "&strIssueNo=" + issueNo+"&strIndentNo="+IndentNo+"&strIndentDate="+IndentDate;
		
		ajaxFunction2(url, "2", "getSpecialIssueDtlsAjaxResponse");
...
function generateIssueReportFunc(obj, i) 
{
	...
	if (issueNo != "0") 
	{
		...
		var url = "OffLineIssueForSpPPCNT.cnt?hmode=" + hmode + "&strMode=2&strStoreId=" + storeId + "&strIssueNo=" + issueNo+"&strIndentNo="+IndentNo+"&strIndentDate="+IndentDate;
		//	alert(url);	
		ajaxFunction2(url, "1", "getSpecialIssueDtlsAjaxResponse");
...
function getSpecialIssueDtlsAjaxResponse(res, mode) 
{
	...
	if (mode == '1') 
	{
		...
		//var itemStockObj2 = document.getElementById("issueDtlsDivId2");
		
		
		itemStockObj.innerHTML = res;
		//itemStockObj2.innerHTML = res;
...
{
	printElement(document.getElementById("issueDtlsDivId"));
}

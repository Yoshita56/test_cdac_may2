		...
		else
		{
			if(document.getElementById("strAvlQtyForChk"+index).value != "0")
			{
					...
					document.getElementById("strIssueUnitId"+index).disabled=true;		
					 ...
					 /*
					  * Set Global Index Here for AUTO
					  * 
					  * */				
					document.forms[0].gblIndex.value=index;
					...
					// div id is a hardcoded value.
							   ...
							   var        hmode = "STOCKDTLSINIT";
								...
								var         url = "IssueToPatientTransBSCNT.cnt?hmode=" + hmode + 
								                  "&strMode="+strMode+
...
function getNAList()
{
					 ...
					 * 9829234234
					 * */
					var   strAdmissionDtlHidVal = document.getElementsByName('strAdmissionDtlHidVal')[0].value;
					
					//console.log("strAdmissionDtlHidVal-1-"+strAdmissionDtlHidVal);	
					console.log("strAdmissionDtlHidVal-2-"+document.getElementsByName('strAdmissionDtlHidVal')[0].value);	
...
*/
	 ...
	 */
	 function cancelStockIpdIssueDtls(obj, index) 
	 {
	 	...
	 	var gbIndexVal = document.forms[0].gblIndex.value;
	 	
	 	document.getElementById("strIssueQty"+gbIndexVal).disabled=false;
	 	
	 	document.forms[0].gblIndex.value="0";
...
function QtyValidation(index)
{
	    ...
	    var   avlQty     = document.getElementById("strAvlQtyBaseVal"+index).value;
		...
		if(document.getElementById("strIssueQty"+index).value!="")
		{
				...
				{
					alert("Issue Quantity  "+parseInt(issueQty,10)+" must be less than or equal Prescribe Quantity "+parseInt(reqQty)+" ");					
					document.getElementById("strIssueQty"+index).value=reqQty;
					return false;
				}				 
				if(parseInt(avlQty)<parseInt(issueQty,10))
				{
					alert("Issue Quantity  "+parseInt(issueQty,10)+" must be less than Avl. Quantity "+parseInt(avlQty)+" ");			
...
function getItemDetails() {
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode=ITEMDETAILS&reqNo=" + gblObject
...
function getConsultantCombo() {
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode=PRESCRIPEDBY&unitCode="
...
function getUnitCombo() {
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode=UNITCMB&deptCode="
...
function getRequestDetail(obj) {
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode=REQUESTDTLS&reqNo=" + obj.value
...
function getItemCat() {
	...
	if (strStoreId != null && strStoreId != "0") {
		var url = "IssueToPatientTransBSCNT.cnt?hmode=ITEMCATCMB&storeId="
...
function getIssuePopUp(these, index) 
{
	...
	$(these).closest("div").find("i").addClass("fa-folder-open");
		...
		objGlobal = these;
		var url = "IssueToPatientTransBSCNT.cnt?hmode=ISSUEDTLPOPUP&strId="
				+ document.getElementById("strIssueSoreID"+index).value + "&issueNo="
				+ document.getElementById("strIssueNo" + index).value+"&viewMode=3";
		//alert(url);
		ajaxFunction(url, "3");
...
//		alert(res);
	...
	{
		//alert(res);
		var itemStockObj = document.getElementById("onLineReqDtl");		
		itemStockObj.innerHTML = res;	
		document.getElementById("patSearchId").style.display = "";	
...
function getViewItemDtl() 
{
	...
	else if (document.getElementsByName("strCrNo")[0].value == "0" || document.getElementsByName("strCrNo")[0].value == "") 
	{
	...
	else 
	{
		...
		var mode = "GETCRDTLS";
		var url = "IssueToPatientTransBSCNT.cnt?hmode=" + mode + "&storeId="
				+ temp + "&itemCatNo="+document.forms[0].strCrNo.value+"&fromDate="
...
function getPatientList()
{
	 ...
	 var storeId       = document.forms[0].strStoreId.value;
	 var strDeptCode       = document.forms[0].strDeptCode.value;
	 // alert(storeId);
	 if(parseInt(storeId)!=0)
	 { 
	   var mode="GET_PAT_LIST";
	   var url="IssueToPatientTransBSCNT.cnt?hmode="+mode+"&storeId="+storeId+"&strDeptCode="+strDeptCode;
	   //alert(url);
	   ajaxFunction(url,"22");
	 }
	 else
...
function validateTariff()
{
	...
	var itemParVal     = document.getElementsByName("itemParamValue");
	 ...
	 var url = "IssueToPatientTransBSCNT.cnt?hmode=TARIFFCHK&itemTmp="+ itemTmp;
	ajaxFunction(url, "11");
...
{	
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode=GET_PAT_ACC_STATUS&crNo="+document.forms[0].strCrNo.value+"&strId="+ document.forms[0].strStoreId.value.split("^")[0];
	ajaxFunction(url, "19");
...
{		
   ...
   var pkey  = document.getElementById("arrBasePkey"+index).value;   
   // P_KEY [ A.HRGNUM_EPISODE_CODE || '^' || A.HRGNUM_VISIT_NO||'^'||A.HRGNUM_PUK ]   
   document.forms[0].strCrNo.value = pkey.split("^")[2];  
   var url = "IssueToPatientTransBSCNT.cnt?hmode=GET_PAT_ACC_STATUS&crNo="+document.forms[0].strCrNo.value+"&strId="+ document.forms[0].strStoreId.value.split("^")[0];
   ajaxFunction(url, "19");
...
function getReport()
{
	 var printReq      = document.forms[0].printReq.value; 
	 var transferNo    = document.forms[0].strIssueNumber.value;	                        
	 var storeId       = document.forms[0].strStoreId.value;

	// alert("Issue_no--"+transferNo+"--printReq--"+printReq+"---Store_id--"+storeId);
	 
	 if(parseInt(transferNo)!=0 && document.forms[0].printReq.value == 1)
	 { 
	   var mode="ISSUEDTLSINITONE";
	   var url="IssueToPatientTransBSCNT.cnt?hmode="+mode+"&strIssueNo="+ transferNo+"&strStoreId="+storeId+"&strMode=1";
	   ajaxFunction(url,"9");
	 }
}
...
function getReport2()
{
	 ...
	 { 
	   ...
	   var url="IssueToPatientTransBSCNT.cnt?hmode="+mode+"&issueNo="+ transferNo+"&storeId="+storeId+"&strCrNo="+crNo+"&strMode=3";
	   //alert(url);
	   ajaxFunction(url,"8");
	 }
...
{	
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode=ISSUEDTLPOPUP&storeId="
				+ strStoreId + "&issueNo="
				+ strIssueNo+ "&strCrNo="
				+ strCrNo+"&strMode=1";
		//alert(url);
...
function checkMsg() {
	...
	if(document.forms[0].crNo.value!="0")
	{
		document.forms[0].strCrNo.value = document.forms[0].crNo.value;
	}
...
function getReqType(){
	//alert('11');
	var url = "IssueToPatientTransBSCNT.cnt?hmode=GETREQTYPE&storeId=" + document.forms[0].strStoreId.value+"&itemCatId=" + document.forms[0].strItemCat.value+"&patStatus="+ document.forms[0].strPatStatus.value ;
	//alert(url);
	ajaxFunction(url,"12");
}

function getPresData(){
	var url = "IssueToPatientTransBSCNT.cnt?hmode=GETPRESCDATA&storeId=" + document.forms[0].strStoreId.value+"&strCrNum=" + document.forms[0].strCrNo.value+"&deptID=" + document.forms[0].strDeptCode.value+"&strDeptUnit=" + document.forms[0].strUnitCode.value+"&episodeDtl=" + document.forms[0].strOffLineEpisode.value;
	//alert(url);
	ajaxFunction(url,"18");
}
...
function checkEpidoseData()
{
	...
	var itemTmp1 = itemTmp.substring(0, itemTmp.length - 1) ; 
	 ...
	 var url = "IssueToPatientTransBSCNT.cnt?hmode=ALREADYISSUEDRUG&itemTmp="+ itemTmp1+"&strCrNum="+document.forms[0].crNo.value+"&episodeDtl="+document.forms[0].strOffLineEpisode.value;
...
function getEpisodeList(obj){
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].crNo.value+"&deptCode="+obj.value+"&unit="+document.forms[0].strUnitCode.value;
	ajaxFunction(url,"16");
...
{    
	...
	{	  
		...
		{		   
		    ...
		    var url = "IssueToPatientTransBSCNT.cnt?hmode=" +hmode+ "&strMode=" +strMode+ "&strStoreId=" +strId+ "&strIssueNo=" + issueNo;
		    ajaxFunction(url, "8");
		}
	}
...
function getPresForm(){
	...
	var url = "IssueToPatientTransBSCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].crNo.value+"&episodeCode="+document.forms[0].strOffLineEpisode.value;
	ajaxFunction(url,"17");
	 				  ...
	 				  //alert("Length-->>"+issueQty.length);
	 				     ...
	 				     // alert("count-->>"+count);	 
	 		              ...
	 		              {	 
		 		              ...
		 		              if(conf2 == true)
		 		              { 
		 		            	  document.forms[0].hmode.value = "NEW_IPD_ISSUE";
		 		            	  document.forms[0].submit();
		 		              }
	 		              }

...
function getIssuePopUp_Org(these, index) 
{
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=ISSUEDTLPOPUP&strId="
...
/*function getPrevIssueDtl_OffLine() {
	...
	if (strFromStoreId != 0) {
		var url = "IssueToPatOPDTransCNT.cnt?hmode=PREVISSUEDTL"
				+"&strId="
...
function getItemCat() {
		...
		var url = "IssueToPatOPDTransCNT.cnt?hmode="+mode+"&storeId="
...
/*	if (document.getElementsByName("strStoreId")[0].value == "0") {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=" + mode+ "&strId="
...
function getAjaxResponse(res, mode) {
	...
	if (mode == "11") {
		...
		var hisValidator = new HISValidator("issuePatBean");
	...
	if (mode == "19") {
		if (res == "") {
			alert("No Billing Record Found For CR No [ "
...
function getDrugNameSelected_off_Line(itemId) {
	...
	document.getElementById("strBatch1-" + (indxRow + 1)).innerHTML = (itemId.split("#")[0]).split("^")[10];
	document.getElementById("strAvlQty1-" + (indxRow + 1)).innerHTML = (itemId.split("#")[1]).split("^")[0];
	document.getElementById("strRate1-" + (indxRow + 1)).innerHTML = (itemId.split("#")[1]).split("^")[1];
...
function getItemDetails() {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=ITEMDETAILS&reqNo=" + gblObject
...
function getConsultantCombo() {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=PRESCRIPEDBY&unitCode="
...
function getUnitCombo() {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=UNITCMB&deptCode="
...
function getRequestDetail(obj) {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=REQUESTDTLS&reqNo=" + obj.value
...
function validateTariff() {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=TARIFFCHK&itemTmp=" + itemTmp;
	ajaxFunction(url, "11");
}
...
function getPatientAccStatus() {
	...
	/*
	 * Commented By Amit Kumar on Date 17-May-2023 No Billing Status Required
	 * for OPD Patient
	 * 
	 * //document.forms[0].crNo.value = document.forms[0].strCrNo.value;
	 * //document.forms[0].strId.value =
	 * document.forms[0].strStoreId.value.split("^")[0];
	 * //document.forms[0].itemCategory.value =
	 * document.forms[0].strItemCat.value;
	 * 
	 * var url = "IssueToPatOPDTransCNT.cnt?hmode=GET_PAT_ACC_STATUS&crNo="+
	 * document.forms[0].strCrNo.value+"&strId="+
	 * document.forms[0].strStoreId.value.split("^")[0]; ajaxFunction(url,
	 * "19");
	 */
...
function onGoButton() {
	...
	// document.forms[0].itemCatName.value =
	// document.forms[0].strItemCat[document.forms[0].strItemCat.selectedIndex].text;
	document.forms[0].crNo.value = document.forms[0].strCrNo.value;

	document.forms[0].strId.value = document.forms[0].strStoreId.value
			.split("^")[0];
	//added by rj47
	//document.forms[0].modeVal.value = document.forms[0].strMode.value;
	...
	// hisValidator.addValidation("strItemCat", "dontselect=0","Select Item
	// Category Combo");
	...
	if (retVal) {
		...
		if (document.forms[0].strOfflineOPDFlag.value == "0") {
			document.forms[0].hmode.value = "ISSUE_OPD_ONLINE"; // ON-LINE With
																// Prescription
		} else {
			document.forms[0].hmode.value = "ISSUE_OPD_OFFLINE"; // OFF-Line
																	// Without
																	// Prescription
		}
...
function getReport() {
	...
	if (parseInt(transferNo) != 0 && document.forms[0].printReq.value == 1) {
		...
		var url = "IssueToPatOPDTransCNT.cnt?hmode=" + mode + "&strIssueNo="
...
function getReport2() {
	...
	if (parseInt(transferNo) != 0) {
		var mode = "ISSUEDTLPOPUP";
		var url = "IssueToPatOPDTransCNT.cnt?hmode=" + mode + "&issueNo="
...
function getReqType() {
	// alert('11');
	var url = "IssueToPatOPDTransCNT.cnt?hmode=GETREQTYPE&storeId="
...
function getPresData() {
	var url = "IssueToPatOPDTransCNT.cnt?hmode=GETPRESCDATA&storeId="
...
function checkEpidoseData() {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=ALREADYISSUEDRUG&itemTmp="
...
function getEpisodeList(obj) {
	...
	var url = "IssueToPatOPDTransCNT.cnt?hmode=" + hmode + "&crNo="
...
function getReport() {
	...
	if (issueNo != "") {
		if (issueNo != "0") {
			...
			var url = "IssueToPatOPDTransCNT.cnt?hmode=" + hmode + "&strMode="

function validateQty(obj, index) {	
	...
	if (avlQty > 0) {		
		if(sancQty > avlQty){
			alert("Approved Quantity can not be Greater than  Available Quantity [ "+ avlQty + " ] of  store!");
			obj.value = "";					   
			return false;
		}
	} else {
...
function validateSave() {
	...
	var hisValidator = new HISValidator("transferShortageApprovalTransBean");
	...
	if (retVal) {
		...
		alert("Please enter sanction qty for at least one record !!!!");
			return false;
		}
...
function validate1()
{
  if(document.forms[0].strReqTypeId.value!='91')
  {
	    var hisValidator = new HISValidator("transferShortageApprovalTransBean"); 

...
{    
   ...
   var mode="APPROVALPRINT";
   var url="IndentApprovalDeskCNT.cnt?hmode="+mode+"&strChk="+ strChk+"&strReqTypeId="+strReqTypeId;
   ajaxFunction(url,"2");
...
//voucher 
function generateFunc(obj, i) 
{
	indexglobal = i;
	parentPopup = obj;
	var itemIdValue       = document.getElementById('strHLPItemIdValue'+ i).value;
	var storeIdValue      = document.getElementById('strHLPStoreIdValue'+ i).value;
	var toStoreIdValue    = document.getElementById('strHLPToStoreIdValue'+ i).value;
	var itemBrandIdValue  = document.getElementById('strHLPItemBrandIdValue'+ i).value;
	var appLevel          = document.getElementById('strHLPLastAppLevel'+ i).value;
		
	
    var hmode = "VOUCHER";
	
	var url = "IndentApprovalDeskCNT.cnt?hmode=" + hmode + "&itemId=" + itemIdValue
	+ "&storeId=" + storeIdValue + "&tostoreId=" + toStoreIdValue+ "&itemBrandId="+itemBrandIdValue+"&appLevel="+appLevel;
	//alert(url);			
	ajaxFunction(url,"3");
...
function getAjaxResponse(res,mode)
{
	  ...
	  if (mode == "3") 
		{	
		    document.getElementById("stockTableHLPDiv").innerHTML=res;
		    display_popup_menu(parent,"StockTable",'240','220');   
		}
	   if(mode=="11")
	     {

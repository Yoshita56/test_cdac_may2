...
function getPOTypeValues(){
	if(document.forms[0].strItemCat.value!="0" && document.forms[0].strPOTypeId.value!="0"){		
		...
		var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&strContractType="+getContractType()+"&strItemCat="+document.forms[0].strItemCat.value;
		ajaxFunction(url,"1");
	}else{
...
function getPOTypeComboValues(){	
	...
	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&strItemCat="+document.forms[0].strItemCat.value+"&strStoreId="+document.forms[0].strStoreId.value;
	//alert("URL"+url);
	ajaxFunction(url,"6");
}
...
function getRequestDetails(){
	if(document.forms[0].strItemCat.value!="0" && document.forms[0].strPOTypeId.value!="0"){			
		...
		var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&strPOTypeId="+document.forms[0].strPOTypeId.value+"&strItemCat="+document.forms[0].strItemCat.value+"&strStoreId="+document.forms[0].strStoreId.value+"&strPOGrantType="+document.forms[0].strPOTypeId.value.split("^")[1];
...
function getAgentName(){
	...
	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&strItemCat="+document.forms[0].strItemCat.value;
	ajaxFunction(url,"5");
}

function getComponentDetails(){
	var hmode = "GETCOMPONENTDETAILS";
	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&strPOTypeId="+document.forms[0].strPOTypeId.value+"&strItemCat="+document.forms[0].strItemCat.value;
	ajaxFunction(url,"4");
}
...
function getRequestItemDetails(){
	...
	if(retVal){
		...
		var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&strPOTypeId="+document.forms[0].strPOTypeId.value+"&strItemCat="+document.forms[0].strItemCat.value+"&strSupplierId="+document.forms[0].strSupplierId.value+"&strContractType="+getContractType()+"&strRequestIds="+strRequestIds+"&strCurrentDate="+document.forms[0].strCurrentDate.value+"&strReqIdnDate="+strReqIdnDate+"&strCheckData="+strCheckData;
		//alert(url);
		ajaxFunction(url,"3");
		document.getElementById("compilebtnid").style.display="none";
...
{
	//howToShowCompileButton();compilebtnid
	document.getElementById("compilebtnid").style.display="block";
	...
	if(confirm("Are you sure want to save data ???"))
	{
		if(confirm("Are you sure ???"))
		{
			...
			if(!retVal){
				...
				document.forms[0].hmode.value="INSERTDRAFT";
				document.forms[0].submit();
		}
		else
		{
...
function checkOrderQty(_these){
	...
	document.getElementById("PACK"+""+_these.id.substr(6)).value=strUser.split("@")[5];
...
{
	var tax=document.getElementById("TAX"+""+_these.id.substr(4)).value ;
	console.log('tax:::::'+tax);
	...
	var totRate1 =((totRate+ ((totRate*tax)/100))*100)/100;
	document.getElementById("TRATE"+""+_these.id.substr(4)).value=parseFloat(totRate1.toFixed(2));
}

function getTotRate1(_these)
{
	var tax=document.getElementById("TAX"+""+_these.id.substr(3)).value;			//document.getElementById("TAX"+""+_these.id.substr(4)).value ;
	console.log('tax:::::'+tax);
	var totRate =Math.round(( document.getElementById("RATE"+""+_these.id.substr(3)).value * document.getElementById("QTY"+""+_these.id.substr(3)).value)*100)/100;
	var totRate1 =((totRate+ ((totRate*tax)/100))*100)/100;
	document.getElementById("TRATE"+""+_these.id.substr(3)).value=parseFloat(totRate1.toFixed(2));
}
...
function get_item_details(obj,i)
{
	...
	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&ItemID="+document.getElementsByName("strDitemId")[i].value+"&strItemBrandID="+document.getElementsByName("strDitemBrandId")[i].value+"&strRaisingStore="+document.getElementsByName("strTmpRaisingStore")[i].value+"&strReqIds="+document.getElementsByName("strCheckBoxItem")[i].value+"&poTypeId="+document.getElementsByName("strPOTypeId")[0].value;
	ajaxFunction(url,"7");
...
function addSuppiler(obj,i)
{
	...
	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&Index="+(numRows-1)+"&ItemID="+document.getElementsByName("strDitemId")[i].value+"&strItemBrandID="+obj.id.substr(3,8)+"&strRaisingStore="+document.getElementsByName("strTmpRaisingStore")[i].value+"&strReqIds="+document.getElementsByName("strCheckBoxItem")[i].value+"&itemCat="+document.getElementsByName("strItemCat")[0].value+"&contractType="+12+"&suppId="+document.getElementsByName("strDSuppId")[i].value+"&unitId="+document.getElementsByName("strDOrderQtyUnitId")[i].value+"&divThisId=divId"+obj.id.substr(3,8);
	ajaxFunction(url,"8");
...
function compile_supp_wise()
{
	...
	var chklen=0;supp = [],item=[],brand=[],tqty=[], qty = [],rate=[],trate=[],qtyunit=[],tax=[],dgrp=[],mrp=[],pack=[];
	...
	for(var i = 0;i<document.getElementsByName("strCheckBoxItem").length;i++)
	{
		if(document.getElementsByName("strCheckBoxItem")[i].checked)
		{
			...
			pack[i] = document.getElementsByName("strDPackSize")[i].value;
			mrp[i] 	= document.getElementsByName("strDMRPCol")[i].value;
			//if(document.forms[0].tmpPoType.value == '22')
			...
			while(j<document.getElementsByName("strDSuppId").length)
			{
				if(document.getElementsByName("strCheckBoxItem")[i].id.substr(3,8) == document.getElementsByName("strDSuppId")[j].id.substr(6,8))
				{
					...
					{
						supp[j] = document.getElementsByName("strDSuppId")[j].value.split("@")[0]+"^"+item[i]+"^"+brand[i]+"^"+tqty[i]+"^"+document.getElementsByName("strDSuppId")[j].value.split("@")[4];
...
//	else
	...
	//if(document.forms[0].tmpPoType.value == '22')
		objData = { ItemID: item , strItemBrandID: brand , strRaisingStore: document.getElementsByName("strStoreId")[0].value , itemCat: document.getElementsByName("strItemCat")[0].value , poType: document.getElementsByName("strPOTypeId")[0].value , suppId: supp , unitId: qtyunit , tqty:tqty , rate:rate,  trate: trate , qty: qty , tax:tax,poDate: document.getElementsByName("strpoDate")[0].value,grp: dgrp,mrp:mrp,pack:pack};
	//else
	//	objData = { ItemID: item , strItemBrandID: brand , strRaisingStore: document.getElementsByName("strStoreId")[0].value , itemCat: document.getElementsByName("strItemCat")[0].value , poType: document.getElementsByName("strPOTypeId")[0].value , suppId: supp , unitId: qtyunit , tqty:tqty , rate:rate,  trate: trate , qty: qty , tax:tax,poDate: document.getElementsByName("strpoDate")[0].value,grp: dgrp,pack:pack};
...
//	var json_upload = "json_name=" + JSON.stringify({name:"John Rambo", time:"2pm"});
	...
	//if(document.forms[0].tmpPoType.value == "21")
	//	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&ItemID="+item+"&strItemBrandID="+brand+"&strRaisingStore="+document.getElementsByName("strStoreId")[0].value+"&itemCat="+document.getElementsByName("strItemCat")[0].value+"&poType="+document.getElementsByName("strPOTypeId")[0].value+"&suppId="+supp+"&unitId="+qtyunit+"&tqty="+tqty+"&rate="+rate+"&trate="+trate+"&qty="+qty+"&tax="+tax+"&fromDate="+document.getElementsByName("strFromDate")[0].value+"&toDate="+document.getElementsByName("strToDate")[0].value;
	//else
	//	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode;//+"&jsnObj="+jsonText1;
...
function sendData(objData) {
	$.ajax({
	        url:createFHashAjaxQuery( "PODeskGenerateTransBSCNT.cnt?hmode=SUPPLIERWISECOMPILATION"),
	        type: 'POST',
...
function getIndentDetails(indentNo,storeId)
{
	...
	var url = "PODeskGenerateTransBSCNT.cnt?hmode="+hmode+"&indentNo="+indentNo+"&strRaisingStore="+storeId;
	ajaxFunction(url,"90");
}
function selectAll()
{

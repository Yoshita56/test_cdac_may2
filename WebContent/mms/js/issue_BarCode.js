...
{
		document.forms[0].hmode.value = "GETVIEWPAGE";
		document.forms[0].submit();
}
...
function cancelToMainPage(mode)
{
	//alert(mode);
	if(mode==2)
	{
...
function getItemDetails() {
	...
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=ITEMDETAILS&reqNo=" + gblObject
...
function getConsultantCombo() {
	...
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=PRESCRIPEDBY&unitCode="
...
function getUnitCombo() {
	...
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=UNITCMB&deptCode="
...
function getRequestDetail(obj) {
	...
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=REQUESTDTLS&reqNo=" + obj.value
...
function getPrevIssueDtl() {
	...
	var strItemCategory ="10";
	if(strFromStoreId !=0){
		var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=PREVISSUEDTL&strId="
			+ document.forms[0].strId.value + "&itemCategory=10&crNo="
...
function getItemCat() {
	...
	if (strStoreId != null && strStoreId != "0") {
		var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=ITEMCATCMB&storeId="
...
function getIssuePopUp(these, index) 
{
	...
	$(these).closest("div").find("i").addClass("fa-folder-open");
		...
		objGlobal = these;
		var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=ISSUEDTLPOPUP&strId="
				+ document.getElementById("strIssueSoreID"+index).value + "&issueNo="
				+ document.getElementById("strIssueNo" + index).value+ "&strCrNo="
				+ document.getElementById("strCrNo" + index).value+"&viewMode=0";
		//alert(url);
		ajaxFunction(url, "3");
...
function getAjaxResponse(res, mode) {
	...
	if(mode=="11")
	{
		 		...
		 		//	{
		 			//	saveObj.style.display = "none"; 
		 				...
		 				var hisValidator = new HISValidator("issueBarCodeBean");
	...
	if (mode == "23") 
	{
		...
		//alert("passBatch---"+passBatch);
		
		if(parseInt(passBatch.split("^")[0])==0)
		{						
			var searchId = 'strSearchBarCode';
			document.getElementById(searchId).value = "";		
			
			alert("Stock Not Avaliable for Batch "+glbBatch.split("^")[1]+" !!");	
		    setTimeout(function(){document.getElementById("strSearchBarCode").focus();}, 1);
		...
		for (var j = 0; j < len; j++) 
		{
			...
			if (passBatch == cBatch) 
			{
				alert(" Duplicate Drugs  [" + rowBatch[j].value.split("^")[0]+ "]  not allowed !! ");
				var searchId = 'strSearchBarCode';
				document.getElementById(searchId).value = "";
				
				document.getElementById("strSearchBarCode").focus();
				
				return false;
			}
		}

		addRows(new Array('strQtyText'), new Array('t', 't', 't', 't', 't', 't'),'1', '1', 'R');
		...
		ReCalcualte_Amount(2);

	}

}
var glbBatch="";
function getBarCodeDetails(obj) 
{	
	glbBatch = obj.value;
	//console.log(document.forms[0].strSearchBarCode.value);
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=GET_BAR_CODE_DTL&crNo="+document.forms[0].strCrNo.value+
	"&strId="+ document.forms[0].strId.value+"&barCodeValue="+obj.value;
	//alert(url);
	ajaxFunction(url, "23");	
}
...
function getPatientAccStatus(mode) 
{	
	
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=GET_PAT_ACC_STATUS&crNo="+document.forms[0].strCrNo.value+"&strId="+ document.forms[0].strStoreId.value.split("^")[0];
	if(mode=="1")
	{	
	    ajaxFunction(url, "19");
	}
...
function getPreviousIssueDrugList() 
{	
	...
	var hisValidator = new HISValidator("issueBarCodeBean");
	hisValidator.addValidation("strStoreId", "dontselect=0","Select Store  Combo ");
	hisValidator.addValidation("strItemCat", "dontselect=0","Select Category Combo");
	...
	if (retVal) 
	{
		document.forms[0].hmode.value = "PREV_ISSUE_LIST";
		document.forms[0].submit();
...
function repeatQtyValidation(index)
{
	if(document.getElementById("strReqQty"+index).value.length>0)
	{
		// hstnum_itembrand_id ^ HSTSTR_BATCH_NO ^ ROUND(HSTNUM_INHAND_QTY) ^ ROUND(NVL(HSTNUM_RATE,0),2) ^ TO_CHAR(HSTDT_EXPIRY_DATE, 'Mon-yyyy')
		    var issueQty     = document.getElementById("strReqQty"+index).value;
		    var   avlQty     = document.getElementById("strAvlQty"+index).value;
		    var   rate       = document.getElementById("strSalePrice"+index).value; 
		    
			if(parseInt(avlQty)<parseInt(issueQty,10))
			{
				alert("Issue Quantity cannot be greater than Available Quantity [ "+avlQty+" ] ");					
				document.getElementById("strReqQty"+index).value="";
				return false;
			}	
										
			var cost = (rate * issueQty).toFixed(2);
			
			
			document.getElementById("strCost"+index).value= cost;
			document.getElementById("costId"+index).innerHTML= cost;
						
			var netCost = 0;
			var netQty  = 0;			
			var totalQty  = document.getElementsByName('strReqQty');
			var totalCost = document.getElementsByName('strCost');
			var length = totalQty.length;		
			//alert("netCost-"+index+"->>"+netCost);
			for(var i=0;i<length;i++)
			{
				var qtyVal  = totalQty[i].value;
				var costVal = totalCost[i].value;
				//console.log("costVal--"+i+"-->>"+parseInt(costVal)+"--Element.Length--"+totalQty[i].value.length);
				
				if(totalQty[i].value.length>0)
				{	
					netQty  = netQty + parseFloat(qtyVal);
			    ...
			    console.log("-repeatQtyValidation-Total Qty-A->>"+netQty);
			    console.log("Total Cost-->>"+netCost);
						
			    document.getElementById("strNetQtyDivId").innerHTML = netQty;
				document.getElementById("strNetQty").value = netQty;
					
				document.getElementById("strNetCostDivId").innerHTML = "Rs. "+roundValue(netCost,2);
				document.getElementById("strNetCost").value = roundValue(netCost,2);
	...
	else
	{
		...
		document.getElementById("costId"+index).innerHTML= "0.00";
		...
		for(var i=0;i<length;i++)
		{
		    ...
		    console.log("-repeatQtyValidation-Total Qty-B->>"+netQty);
		    console.log("Total Cost-->>"+netCost);
...
{	
	if(document.getElementById("strReqQty"+index).value!="" && obj.value!="0")
	{
		// hstnum_itembrand_id ^ HSTSTR_BATCH_NO ^ ROUND(HSTNUM_INHAND_QTY) ^ ROUND(NVL(HSTNUM_RATE,0),2) ^ TO_CHAR(HSTDT_EXPIRY_DATE, 'Mon-yyyy')
		    var issueQty     = document.getElementById("strReqQty"+index).value;
		    var   avlQty     = obj.value.split("^")[2];
			...
			document.getElementById("expId"+index).innerHTML = obj.value.split("^")[4];
			document.getElementById("avlQtyId"+index).innerHTML= obj.value.split("^")[2];
			document.getElementById("salePriceId"+index).innerHTML= obj.value.split("^")[3];			
			
			document.getElementById("strAvlQty"+index).value = obj.value.split("^")[2];
			document.getElementById("strBatch"+index).value= obj.value.split("^")[1];			
			document.getElementById("strSalePrice"+index).value= obj.value.split("^")[3];
			document.getElementById("strExpDate"+index).value= obj.value.split("^")[4];
	...
	{		
		...
		document.getElementById("strAvlQty"+index).value = "-";
		document.getElementById("strBatch"+index).value= "-";			
		document.getElementById("strSalePrice"+index).value= "0";
		document.getElementById("strExpDate"+index).value= "-";
		
		document.getElementById("expId"+index).innerHTML = "-";
		document.getElementById("avlQtyId"+index).innerHTML= "-";
		document.getElementById("salePriceId"+index).innerHTML= "-";	
						
		var cost = "0";
		
		document.getElementById("costId"+index).innerHTML= "0";
...
function quantityValidation(index)
{
...
function getIssuePopUpFromHLP(these, index) 
{	
	
	var strStoreId = document.getElementById("strHlpStoreId"+index).value;
	var strIssueNo = document.getElementById("strHlpReturnNo"+index).value;
...
function getViewItemDtl() 
{
	...
	else 
	{
		...
		var mode = "GETCRDTLS";
		var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=" + mode + "&storeId="
...
function validateTariff()
{
	...
	var itemParVal     = document.getElementsByName("itemParamValue");
	 ...
	 var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=TARIFFCHK&itemTmp="+ itemTmp;
	ajaxFunction(url, "11");
...
function onGoButtonOld() {
	...
	{
		document.forms[0].strStoreId.disabled=false;
		document.forms[0].hmode.value = "ISSUETOPATCOUNT";
		document.forms[0].submit();
...
function repeatRequest()
{
	...
	if(saveObj.style.display == "" || saveObj.style.display == "block" || saveObj.style.display == "Block" || saveObj.style.display == "BLOCK")
	{
						    ...
						    var reqQtyUnit     = document.getElementsByName("strUnitName");
						    var count = 0;				   
						  ...
						  var conf = confirm("Going To Save Repeat Request For [ "+(parseInt(reqQty.length)-parseInt(count))+" ] Items Out of [ "+parseInt(reqQty.length)+" ] !!");
				          if(conf == true)
				          {
				               var conf1 = confirm("Are you sure !!!");
				               if(conf1 == true)
				               {	              
							              ...
							              document.forms[0].hmode.value="REPEAT_ISSUE_INSERT";
								       	  document.forms[0].submit();
				               }
				              else
				               {
...
function getReport()
{
	...
	// alert("storeId"+storeId);
	 ...
	 { 
	   ...
	   var url="IssueToPatientBarCodeTransCNT.cnt?hmode="+mode+"&strIssueNo="+ transferNo+"&strStoreId="+storeId+"&strMode=1";
	   ajaxFunction(url,"9");
	 }
}
...
function getReport2()
{
	 ...
	 { 
	   ...
	   var url="IssueToPatientBarCodeTransCNT.cnt?hmode="+mode+"&issueNo="+ transferNo+"&storeId="+storeId;
	   ajaxFunction(url,"8");
	 }
...
function checkMsg() 
{
	...
	var pos  = document.getElementById('strCrNo').value;	
	var ctrl = document.getElementById('strCrNo');    
...
function getReqType(){
	//alert('11');
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=GETREQTYPE&storeId=" + document.forms[0].strStoreId.value+"&itemCatId=" + document.forms[0].strItemCat.value+"&patStatus="+ document.forms[0].strPatStatus.value ;
	//alert(url);
	ajaxFunction(url,"12");
}

function getPresData(){
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=GETPRESCDATA&storeId=" + document.forms[0].strStoreId.value+"&strCrNum=" + document.forms[0].strCrNo.value+"&deptID=" + document.forms[0].strDeptCode.value+"&strDeptUnit=" + document.forms[0].strUnitCode.value+"&episodeDtl=" + document.forms[0].strOffLineEpisode.value;
	//alert(url);
	ajaxFunction(url,"18");
}
...
function checkEpidoseData()
{
	...
	var itemTmp1 = itemTmp.substring(0, itemTmp.length - 1) ; 
	 ...
	 var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=ALREADYISSUEDRUG&itemTmp="+ itemTmp1+"&strCrNum="+document.forms[0].crNo.value+"&episodeDtl="+document.forms[0].strOffLineEpisode.value;
...
function getEpisodeList(obj){
	...
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].crNo.value+"&deptCode="+obj.value+"&unit="+document.forms[0].strUnitCode.value;
	ajaxFunction(url,"16");
...
{    
	...
	{	  
		...
		{		   
		    ...
		    var url = "IssueToPatientBarCodeTransCNT.cnt?hmode=" +hmode+ "&strMode=" +strMode+ "&strStoreId=" +strId+ "&strIssueNo=" + issueNo;
		    ajaxFunction(url, "8");
		}
	}
	document.forms[0].strIssueNum.value = "0";	
...
function getPresForm(){
	...
	var url = "IssueToPatientBarCodeTransCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].crNo.value+"&episodeCode="+document.forms[0].strOffLineEpisode.value;
	ajaxFunction(url,"17");

 ...
 */
	...
	function saveSchedule()
	{
		...
		var  itemParVal     = document.getElementsByName("strVerifyCountedQty");				    
	    if (itemParVal.length > 1) 
		{
			for (var i = 0; i < itemParVal.length; i++) 
			{					
				    //console.log("Index"+index+"--seq--"+i);
					if (parseInt(itemParVal[i].value) == "0") 
					{
						alert("Empty Bar Code Not Allowed !!");					
						document.getElementById("strVerifyCountedQty"+i).focus();
						return false;
					}
    	...
    	if(	confirm("You Are Going To Save "))				
		{
				 ...
				 document.forms[0].hmode.value="INSERT_BARCODE";
				 document.forms[0].submit();						
	...
	function getInventoryDtls() 
	{
	   	    ...
	   	    var hisValidator = new HISValidator("barCodeGeneBean");		
			 
			hisValidator.addValidation("strStoreId",        "dontselect=0", "Please Select Store !!" );
			hisValidator.addValidation("strItemCategoryId", "dontselect=0", "Please Select Category !!" );
			...
			var url = "StockBarCodeTransCNT.cnt?hmode="+hmode+	
			...
			"&strItemId="+document.forms[0].strItemId[document.forms[0].strItemId.selectedIndex].value+
			"&strGeneType="+document.forms[0].strGeneType.value+
			"&strBatchNo="+document.forms[0].strBatchNo[document.forms[0].strBatchNo.selectedIndex].value+
			"&strSizeId="+document.getElementById('strSizeId').value;
			
			var action1="StockBarCodeTransCNT.cnt?hmode=Print";
			 $.ajax({
					url : url,
					type : "POST",
					async : true,
			 ...
			 var child = window.open(action1,'popupWindow','status=yes,scrollbars=yes,height=280,width=500,left=10,top=10');  
			...
			/*
			 ...
			 document.forms[0].hmode.value="GOVIEWPAGE";
			 document.forms[0].submit();	
			 */
 ...
 function itemNameCombo()
 {
	...
	objVal = document.getElementById("batchComboDivId");		
	objVal.innerHTML = "<select name ='strBatchNo' id ='strBatchNo' class='browser-default custom-select' onChange='resetDiv();'><option value='0'>All</option></select>";

	var mode = "LPITEMNAME"; 
	var url = "StockBarCodeTransCNT.cnt?hmode="+mode+
 ...
 function resetDiv()
 {
		...
		if(strbatch == 0){
			alert("0")
			document.getElementById("SizeDivId").style.display = "none";
			}else{
			document.getElementById("SizeDivId").style.display = "flex"; 
			} 
		getInventoryDtls(); 
	 
 }
 function getDrugNameSelected_off_Line(itemId,value) 
 {
 	//alert(itemId+"--"+value);
 	
 	document.getElementById("strBItemId").value = itemId;
 	
 	document.getElementById("strSearchDrug").value = "";
 	
 	document.getElementById("drugNameDisplayDivId").style.display="block";
 	
 	document.getElementById("selectDrugNameDivId").innerHTML = value;	 	
 ...
 function getAjaxResponse(res,mode)
 {
	...
	{	
	   ...
	   if(temp1[0] == "ERROR")
	    ...
	    {   
			 ...
			 objVal.innerHTML = "<select name ='strItemId' id ='strItemId' class='browser-default custom-select' onChange='getBatch();'>" + res + "</select>";
			 
			 /*
			 document.forms[0].strSearchDrug.focus();
				...
				$('#strSearchDrug').autocomplete({
					...
					{
						getDrugNameSelected_off_Line(suggestion.data,suggestion.value);
					}
				});
	...
	if (mode == "13") {
        //alert(""+res);
		objVal = document.getElementById("batchComboDivId");		
		objVal.innerHTML = "<select name ='strBatchNo' id ='strBatchNo' class='browser-default custom-select' onChange=''>" + res + "</select>";
	}
	if (mode == "1") {
...
//		document.getElementById("tb12").innerHTML = ""; 
	    ...
	    document.getElementById("strItemId").disabled = true;
	    document.getElementById("strBatchNo").disabled = true;
//	    document.getElementById("strSizeId").disabled = true;
	    if(genType=="1")
	    {	
	        generateBarCode();
	        document.getElementById("saveBarCodeId").style.display = "none";
	    }
	    else
	    {
	    	document.getElementById("saveBarCodeId").style.display = "block";
	    }
...
// 		document.getElementById("giftedNewItemDivId").style.display = "none";
  ...
  function getNewReceving(chkValue) 
  {
			...
			var url = "LocalPurchaseMRN_CNT.cnt?hmode="+hmode+"&storeId="+chkValue;
 			ajaxFunction(url,"10");
...
function pageResetMethod()
{
 ...
 function onClearButton()
 {
		...
		x.style.display = "none";
		y.style.display = "none";
		document.getElementById("viewCancelButtonDivId1").style.display = "none";
		objVal.innerHTML = "";
 ...
 */
 function getSubGroupListDtls(grpObj) {
	...
	var url = "LocalPurchaseMRN_CNT.cnt?hmode="+mode+"&ItemId="
			+ grpObj.value ;

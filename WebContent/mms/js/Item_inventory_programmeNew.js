		 	...
		 	{   
		 	 ...
		 	 generateSlNo();
			 	...
			 	for(var j=0; j<document.getElementsByName("strMultiRowExistingBatchId").length;j++)
			 	{
			 		...
			 		document.getElementsByName("strMultiRowRate")[j].value = document.getElementById("strMultiRowItemId").value.split("^")[4].split("#")[0];			 		
...
{	
   ...
   var saveObj = document.getElementById("saveId");
	...
	{	
       ...
       var hisValidator = new HISValidator("itemInventoryProgramTransBean");
  	...
  	hisValidator.addValidation("strMultiRowCosttoPat","req","Please enter Cost to Patient" );
  	   if(document.forms[0].strBatchMandFlg.value=='1')
...
function getRateUnitCmb()
{
    ...
    {	
	  var mode = "RATEUNIT";
	  var url = "ItemInventoryWithProgramTransCNT.cnt?hmode="+mode+"&strItemBrandId="+ document.getElementById("strMultiRowItemId").value.split('#')[0]+ "&storeId="+document.forms[0].strStoreId.value+"&index=1-1";
	  //alert(url);
      ajaxFunction(url, "2");
   }	
}
...
{  
  ...
  {  
	  ...
	  document.getElementById("strMultiRowPurRate"+index).value=obj.value.split("#")[6];
	  ...
	  document.getElementById("strMultiRowHandlingCharges"+index).value=obj.value.split("#")[18];
	  document.getElementById("strMultiRowHandlingChargesFinal"+index).value=parseFloat(obj.value.split("#")[14])-parseFloat(Math.round((parseFloat(obj.value.split("#")[6])+((parseFloat(obj.value.split("#")[6])*parseFloat(obj.value.split("#")[19]))/100))*100)/100);
	  ...
	  document.getElementById("strMultiRowTax"+index).value=obj.value.split("#")[19];
	  document.getElementById("strMultiRowRate"+index).value= Math.round((parseFloat(obj.value.split("#")[6])+((parseFloat(obj.value.split("#")[6])*parseFloat(obj.value.split("#")[19]))/100))*100)/100;
  ...
  else
  {
      ...
      document.getElementById("strMultiRowInActiveQty"+index).disabled = false;
      document.getElementById("strMultiRowCosttoPat"+index).disabled = false;
      document.getElementById("strMultiRowPurRate"+index).disabled = false;
      document.getElementById("strMultiRowPurRate"+index).value="";
...
function ajaxExistingBatchDtl() 
{
	 ...
	 */
	 function addFreeItems() {
	 	...
	 	getItemOtherDetailsView(mode, isBatchReq , isExpDtReq , '1' , "itemInventoryProgramTransBean");
...
function calCost(obj,index)
{
	...
	var costtopat=parseFloat(document.getElementById("strMultiRowRate"+index).value)+(parseFloat(document.getElementById("strMultiRowPurRate"+index).value) * (parseFloat(obj.value)/100));
	...
	if(obj.value != '')
	{
		...
		document.getElementById("strMultiRowHandlingChargesFinal"+index).value=(parseFloat(document.getElementById("strMultiRowPurRate"+index).value) * (parseFloat(obj.value)/100));
		if((parseFloat(document.getElementById("strMultiRowPurRate"+index).value) * (parseFloat(obj.value)/100)) > 1000)
		{
...
function calSalePrice(obj,index)
{
	...
	{
		document.getElementById("strMultiRowRate"+index).disabled=false;
		var costtopat=parseFloat(document.getElementById("strMultiRowPurRate"+index).value)+(parseFloat(document.getElementById("strMultiRowPurRate"+index).value) * (parseFloat(obj.value)/100));
		//alert("costtopat"+costtopat);
		document.getElementById("strMultiRowRate"+index).value=Math.round(costtopat*100)/100;
		document.getElementById("strMultiRowRate"+index).disabled=true;
		calCost(document.getElementById("strMultiRowHandlingCharges"+index),index);
	}
	else
	{
		document.getElementById("strMultiRowRate"+index).disabled=false;
...
function calSalePriceNew(obj,index)
{
	//alert(obj.value);
	//alert(index);
	//alert(document.getElementById("strMultiRowPurRate"+index).value);
	...
	{
		document.getElementById("strMultiRowRate"+index).disabled=false;
		var costtopat=parseFloat(obj.value)+(parseFloat(obj.value) * (parseFloat(document.getElementById("strMultiRowTax"+index).value)/100));
		//alert("costtopat"+costtopat);
		document.getElementById("strMultiRowRate"+index).value=Math.round(costtopat*100)/100;

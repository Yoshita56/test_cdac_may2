...
{	
	shiftToRightLimit1("strLeftItemIds","strRightItemIds",1,25);
...
{        
    shiftToLeft1("strLeftItemIds","strRightItemIds",1);
    $('#strItemId').html($('#strLeftItemIds').html()); 
    //loadAutocompleteItems();         
}

function shiftToRightLimit1(leftItemName, rightItemName, removeInsert, limitQty)
{    
...
function shiftToLeft1(left_list_name,right_list_name,left_list_insert)
{
	var index=0;
	var count=0, i = 0;
	var lobj,robj;
...
function shiftToRight1(left_list_name,right_list_name,left_list_remove)
{
	var index=0;
	var count=0, i = 0;
	var lobj,robj;
...
function getItemCatCmb(){ 
		...
		if(document.forms[0].strStoreId.value!=0){
			var url ="CovidStockLedgerRptCNT.cnt?hmode=ITEMCATCMB&storeId="+document.forms[0].strStoreId.value;
	 		ajaxFunction(url,"1");
	 	}else{
...
/*function getGroupCmb(){ 
	...
	if(document.forms[0].strItemCatNo.value!=0){
		var url ="CovidStockLedgerRptCNT.cnt?hmode=GROUPCMB&itemCatId="+document.forms[0].strItemCatNo.value;
 		ajaxFunction(url,"2");
 		}else{
...
function getItemCmb(){ 
	...
	if(document.forms[0].strGroupId.value!=0){
		var url ="CovidStockLedgerRptCNT.cnt?hmode=ITEMCMB&" +
				"&storeId="+document.forms[0].strStoreId.value+"&itemCatId="+document.forms[0].strItemCatNo.value;
 		ajaxFunction(url,"3");
...
function getAjaxResponse(res,mode){
	...
	if(mode=="3"){ 
			...
			objVal.innerHTML = "<select id='strItemId' name ='strItemId' class='comboNormal' >"+res+"</select>";		
...
function validate()
{
	    ...
	    var hisValidator = new HISValidator("covidStockLedgerRpt");
	
		if( (document.getElementsByName("strStoreId")[0].value=='-1') )
		{
		...
		{
			alert("Please Select atleast one Drug Name");
		...
		if(retVal){
					...
					var url="CovidStockLedgerRptCNT.cnt?hmode="+mode+
 ...
 * 
 */
function printDetailedStockLedgerReport(obj)
{	  
...
function getDrugName()
{
	    ...
	    var url ="CovidStockLedgerRptCNT.cnt?hmode=DRUGNAME&storeId="+document.forms[0].strStoreId.value+
...
function view2(id1,id2,id3,id4)
{
	document.getElementById("storeDtlsDivId").innerHTML	= "Store : "+document.forms[0].strStoreId[document.forms[0].strStoreId.selectedIndex].text
															+" &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp		Date Range : "+document.forms[0].strFromDate.value
															+" To " +document.forms[0].strToDate.value;
...
function onBatchClick(obj,index)
{
	...
	//  alert(hiddenVal);
	  ...
	  parentPopUp = obj;
	  document.getElementById("1").innerHTML="Item Name : "+hiddenVal.split("^")[0]+", Batch No : "+hiddenVal.split("^")[1];
	  document.getElementById("2").innerHTML=hiddenVal.split("^")[2];
	  document.getElementById("3").innerHTML=hiddenVal.split("^")[3];
	  
	display_popup_menu(parentPopUp, 'issueDtlId', '300', '');
}

...
function getItemCategorys(strCombo) {
	...
	var url = "GiftedItemDetailsTransCNT.cnt?hmode=" + hmode + "&storeId="
			+ strCombo.value;
...
//	hisValidator.clearAllValidations();
	...
	if (retVal) {
		...
		var url = "GiftedItemDetailsTransCNT.cnt?hmode="
				+ hmode
				+ "&storeId="
...
function getSubGroupListDtls(grpObj) {
	...
	var url = "GiftedItemDetailsTransCNT.cnt?hmode=" + mode + "&GroupId="
			+ grpObj.value;
...
function ajaxItemBrandName1(mode) {
	...
	var url = "GiftedItemDetailsTransCNT.cnt?hmode=" + mode + "&strItemId="
...
function ajaxManufectureName(mode) {
	...
	var url = "GiftedItemDetailsTransCNT.cnt?hmode=MANUFECTURENAME&strItemBrandId="
...
//       alert(res);
		...
		objVal.innerHTML = "<select name ='strUnitRateID' class='comboNormal' onChange='getIssueRateUnitCombo();' >"
				+ res + "</select>";
		document.getElementById("UnitRateID1").innerHTML = "<select name ='strUnitSaleID' class='comboNormal' onchange='return validateQty(\"strInHandQuantity\",\"strUnitSaleID\");' disabled='disabled'>"
				+ res + "</select>";
	...
	if (mode == "6") {
		...
		objVal.innerHTML = "<select name='strItemCategoryId' onchange='reSetViewDetails();' class='comboNormal'>"
				+ res + "</select>";
	...
	if (mode == "8") {
		...
		objVal.innerHTML = "<select name='strSubGroupId' class='comboNormal' onChange='ajaxItemName(\"ITEMNAME\");'>"
				+ res + "</select>";
...
{
//	console.log("checking for setWarrantyManfacturer");

...
<script type="text/javascript">
	...
	function ReCalcualte_Amount(mode) 
	{
		var j = 0;
		var i = 0;
		var Total = 0;
	...
	function QtyValidationBeforeDelete_offline(index) 
	{
		...
		if(parseInt(document.forms[0].strScanVar.value)==1)
		{
		   document.getElementById("strSearchDrug").focus();
		}
	    else
		...
		/*
	function barCode(eve) {
		var flag = validateData(eve, 5);
		if (flag) {
			if (eve.keyCode == 13) {
				//onGoButton();
				console.log($("#strSearchBarCode").val());
				getBarCodeDetails($("#strSearchBarCode").val());
			}
		} else {
			return false;
		}
	...
	*/
    ...
    function handleBarcodeScanned(event) 
    {
        ...
        document.getElementById('strSearchBarCode').value = barcode;    // Update the text box with the scanned barcode
      
        // Optionally, you can trigger a click event here if needed
        // document.getElementById('submitButton').click();
    }

    // Add event listener for input change
    document.getElementById('strSearchBarCode').addEventListener('input', handleBarcodeScanned);
    ...
    function testClick(obj)
    {
    	...
    	if(parseInt(obj.value)==1)
    		{
    		   document.getElementById("normalIssue").style.display = "block";
    		   document.getElementById("barCodeIssue").style.display = "none";
    		   document.forms[0].strScanVar.value="1";
    		   document.getElementById("strSearchDrug").focus();
    		}
    	else
    		{
    		    document.getElementById("barCodeIssue").style.display = "block";
    		    document.getElementById("normalIssue").style.display = "none";
    		    document.forms[0].strScanVar.value="2";
    		    document.getElementById("strSearchBarCode").focus();
    		}
    	//barCodeIssue, normalIssue
    }
...
<html:form name="issueBarCodeBean" action="/transactions/IssueToPatientBarCodeTransCNT" type="mms.transactions.controller.fb.IssueToPatientBarCodeTransFB">
	...
	<div class="legend2" id='nonPrintableLegend2'>
  	...
  	<div class="container-fluid">		
		...
		<pDtl:patDtlNew crNo="${issueBarCodeBean.crNo}" address="false"></pDtl:patDtlNew>
				
		<div id="strAdmDtl">
			<bean:write name="issueBarCodeBean" property="strAdmDtl" filter="false" />
		</div>
		...
		<div class="prescriptionTile">
			...
			<div class="row rowFlex reFlex">
				...
				<div class="col-sm-6"  style="display: none;">						
					<select name="strStoreId" onchange="   " class="browser-default custom-select">
						<bean:write name="issueBarCodeBean" property="strStoreValues" filter="false" />
					</select>
				</div>
			</div>
			...
			<div class="collapse" id="TreatmentId">
				<div class="row newrow2"></div>
					<bean:write name="issueBarCodeBean" property="strPatientTreatmentDtl"	filter="false" />
				<div class="row newrow2"></div>			
				<br>
			</div>
			...
			<div class="row rowFlex reFlex" style="display:none;">
				...
				<div class="col-sm-2">									
					<select id='dep' name="strDeptCode" onchange="getUnitCombo();" class="browser-default custom-select">
						<bean:write name="issueBarCodeBean" property="strDeptValues" filter="false" />
					</select>
				</div>
				<div class="col-sm-2">
				...
				<div class="col-sm-2" id="unitDivId">										
					<select	id='unit' name="strUnitCode" onchange="getEpisodeList(document.getElementById('dep'))" class="browser-default custom-select" >
						...
						<bean:write name="issueBarCodeBean" property="strUnitValues" filter="false" />
					</select>
				</div>				
			...
			<div class="row rowFlex reFlex">			
				...
				<div class="col-sm-2" style="">						 
						<div class="form-check">
					     ...
					     <input type="hidden" class="form-control" name="strPrescribedFor" maxlength="3" value="1" onkeypress="return validateData(event,5);">				
				</div>
				...
				<div class="col-sm-2">
				<%-- <date:date name="strPrescriptionDate" value="${issueBarCodeBean.strCtDate}"></date:date> --%>

					<input  style='font-size: 14px; font-weight: 400; color: #495057;' class="form-control datepicker1" value="${issueBarCodeBean.strCtDate}" name="strPrescriptionDate">	
				</div>
			</div>
			...
			<div id='strPresData'>
				<bean:write name="issueBarCodeBean" property="strOnlineTreatment"	filter="false" />
			</div>		
		
			<!-- New Code Start Here -->
			<!-- onkeypress='return barCode(event);' -->
			
			<div id='barCodeIssue' style='display:none;'><input class='form-control' type="text" id="strSearchBarCode" name="strSearchBarCode"  placeholder="Scan Bar Code Here" onchange="getBarCodeDetails(this)" style="border:1px solid;" size="120%"/></div>
			
			<div id='normalIssue'><input class='form-control' type="text" id="strSearchDrug" name="strSearchDrug" style="border:1px solid;" placeholder="Enter first 3 characters of Drug Name to Search" size="120%"/></div> 
				
				<div style='display: none;'>							
					<select name="strMultiRowItemId"     id="strMultiRowItemId" class="form-control form-control-sm" >										       
					     <bean:write name="issueBarCodeBean" property="strItemNameValues" filter="false" />
					</select> 
				</div>
			...
			<div class="row rowFlex reFlex" style='margin: 5px;'>			
				<logic:equal value="1"  name="issueBarCodeBean" property="strPatStatus">			
					<div class="col-sm-2">
						<label>Payment Mode </label>
					</div>
					<div class="col-sm-2">
						<select	id='strPayMode' name="strPayMode"  class="browser-default custom-select" >
							<bean:write name="issueBarCodeBean" property="strPayModeValues" filter="false" />
						</select>
					</div>
					<div class="col-sm-2">
				...
				<logic:equal value="2"  name="issueBarCodeBean" property="strPatStatus">
	...
	<logic:equal value="1" name="issueBarCodeBean" property="strMode">
		<table class="TABLEWIDTH" align="center" border="0" cellspacing="1px">
			<tr class="HEADER">
	...
	<logic:equal value="2" name="issueBarCodeBean" property="strMode">
		<table class="TABLEWIDTH" align="center" border="0" cellspacing="1px">
			<tr class="HEADER">
		...
		<table class="TABLEWIDTH" align="center" cellpadding="1px" cellspacing="1px">
			...
			<tr>
				...
				<td width="25%" colspan="1" class="CONTROL"><bean:write
					name="issueBarCodeBean" property="strLFAccountOpenDate" filter="false" /></td>
		
				<td width="25%" colspan="1" class="LABEL">Current Balance</td>
				<td width="25%" colspan="1" class="CONTROL"><bean:write
					name="issueBarCodeBean" property="strLFBalanceAmount" filter="false" /></td>
			</tr>
			<tr>
		
				<td width="25%" colspan="1" class="LABEL">LF Account Status</td>
				<td width="25%" colspan="3" class="CONTROL"><bean:write
					name="issueBarCodeBean" property="strLFAccountStatus" filter="false" /></td>
			</tr>
		</table>
	</div>
	...
	<div style="display: none;">
		<div id="patientDetailsDivId" style="display: none;">
			<table class="TABLEWIDTH" align="center" cellspacing="1px">
				<tr>
					<bean:write name="issueBarCodeBean" property="strPatientDetails" filter="false" />
				</tr>
			</table>
			...
			<div id="patientDiagDetailsDivId" style="display: block;">
				<table class="TABLEWIDTH" align="center" cellspacing="1px">
					<tr>
						<bean:write name="issueBarCodeBean" property="strPatientDiagDetails" filter="false" />
					</tr>
				</table>
			</div>
		</div>
	...
	<input type="hidden" name="strCrNo" 				value="${issueBarCodeBean.strCrNo}" />		
	...
	<input type="hidden" name="strIssueNo" 				value="${issueBarCodeBean.strIssueNo}" />
	<input type="hidden" name="strIssueDtl"				value="${issueBarCodeBean.strIssueDtl}" />
	<input type="hidden" name="disFlag" 				value="${issueBarCodeBean.disFlag}" />
	...
	<input type="hidden" name="strCtDate" 				value="${issueBarCodeBean.strCtDate}" />
	...
	<input type="hidden" name="strScanVar" 				value="1" />
	<input type="hidden" name="strErrMsg" 				value="${issueBarCodeBean.strErrMsg}" />
	<input type="hidden" name="strBillingHiddenValue" 	value="${issueBarCodeBean.strBillingHiddenValue}" />
	<input type="hidden" name="strTariff_Flag" 			value="1" />
	<input type="hidden" name="strPatStatus" 			value="${issueBarCodeBean.strPatStatus}" />
	<input type="hidden" name="strRowCount" 			value="${issueBarCodeBean.strRowCount}" />
						...
						<div class="modal-body" id="modalSpaceId">
							<div  id='payDtlCDMenuInner ' style="display: block">
								...
								<div class="row rowFlex reFlex">
									...
									<div class="col-sm-6"><input type="text" class="form-control" tabindex="1" maxlength="11" placeholder="(DD-Mon-YYYY)" name="strAuthDate" value="${issueBarCodeBean.strCtDate }"  style="border:0.5px solid black;"/></div>
								</div>
								<div class="row rowFlex reFlex">
								...
								<div class="row rowFlex reFlex">
									...
									<div class="col-sm-6">
										<input type="text" tabindex="1" placeholder="(DD-Mon-YYYY)" class="form-control"
											...
											value="${issueBarCodeBean.strCtDate }" style="border:0.5px solid black;"/>
									</div>
								</div>
							</div>
						</div>
...
<jsp:include page="isse_BarCode_trans_search_row.jsp"></jsp:include>

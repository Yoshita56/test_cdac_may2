...
<html:form name="issueOrgBean" action="/transactions/IssueToPatientTransBSOCNT" type="mms.transactions.controller.fb.IssueTransOFB">
	...
	<div class="legend2" id='nonPrintableLegend2'>
		...
		<!-- <button  id="printbutton" type="button" class="btn btn-outline-primary mt-1 btn-circle printbtn" data-toggle="modal" data-target="" style="background-color:#2e79b4;" onclick="showSlip();" style="display: none;">
  	...
  	<div class="container-fluid">		
		...
		<pDtl:patDtlNew crNo="${issueOrgBean.crNo}" address="false"></pDtl:patDtlNew>
				
		<div id="strAdmDtl">
			<bean:write name="issueOrgBean" property="strAdmDtl" filter="false" />
		</div>
		...
		<div class="prescriptionTile">
			...
			<div class="row rowFlex reFlex">
				...
				<div class="col-sm-6"  style="display: none;">						
					<select name="strStoreId" onchange="   " class="browser-default custom-select">
						<bean:write name="issueOrgBean" property="strStoreValues" filter="false" />
					</select>
				</div>
			</div>
			...
			<div class="collapse" id="TreatmentId">
				<div class="row newrow2"></div>
					<bean:write name="issueOrgBean" property="strPatientTreatmentDtl"	filter="false" />
				<div class="row newrow2"></div>			
				<br>
			</div>
			...
			<div class="row rowFlex reFlex">
				...
				<div class="col-sm-2">									
					<select id='dep' name="strDeptCode" onchange="getUnitCombo();" class="browser-default custom-select">
						<bean:write name="issueOrgBean" property="strDeptValues" filter="false" />
					</select>
				</div>
				<div class="col-sm-2">
				...
				<div class="col-sm-2" id="unitDivId">										
					<select	id='unit' name="strUnitCode" onchange="getEpisodeList(document.getElementById('dep'))" class="browser-default custom-select" >
						...
						<bean:write name="issueOrgBean" property="strUnitValues" filter="false" />
					</select>
				</div>				
			...
			<div class="row rowFlex reFlex">			
				...
				<div class="col-sm-2">
				<%-- <date:date name="strPrescriptionDate" value="${issueOrgBean.strCtDate}"></date:date> --%>

					<input  style='font-size: 14px; font-weight: 400; color: #495057;' class="form-control datepicker1" value="${issueOrgBean.strCtDate}" name="strPrescriptionDate">	
				</div>
			</div>
			...
			<div id='strPresData'>
				<bean:write name="issueOrgBean" property="strOnlineTreatment"	filter="false" />
			</div>		
			
			<div class="">			
				<table class="table"  border="1px">
					<thead>
						<th  style='text-align:center;' width="32%"><b>Item Name</b></th>
						<th  style='text-align:center;' width="15%"><b>Batch No</b></th>
						<th  style='text-align:center;' width="10%"><b>Available Qty</b></th>
						<th  style='text-align:center;' width="15%"><b><font color="red">*</font>Cost/Unit</b></th>
						<th  style='text-align:center;' width="10%"><font color="red">*</font><b>Quantity</b></th>
						<th  style='text-align:center;' width="8%"><font color="red">*</font><b>Cost</b></th>
						<th  style='text-align:center;' width="10%">
							<button type="button" class="btn btn-info btn-sm btnbg" tabindex="2" id='strSearchItemButtonDivId' onclick='getItemSelectPopup();' name="searchName" style="float: right;">
			   				<span class="btn-label"></span><i class="fas fa-search-plus"></i>&nbsp;Item Finder
			   			</button>
			   			</th>
					</thead>
					<!-- <tbody><tr></tr></tbody> -->
			...
			<div class="row rowFlex reFlex" style='margin: 5px;'>			
				<logic:equal value="1"  name="issueOrgBean" property="strPatStatus">			
					<div class="col-sm-2">
						<label>Payment Mode </label>
					</div>
					<div class="col-sm-2">
						<select	id='strPayMode' name="strPayMode"  class="browser-default custom-select" >
							<bean:write name="issueOrgBean" property="strPayModeValues" filter="false" />
						</select>
					</div>
					<div class="col-sm-2">
				...
				<logic:equal value="2"  name="issueOrgBean" property="strPatStatus">
		...
		<table class="TABLEWIDTH" align="center" cellpadding="1px" cellspacing="1px">
			...
			<tr>
				...
				<td width="25%" colspan="1" class="CONTROL"><bean:write
					name="issueOrgBean" property="strLFAccountOpenDate" filter="false" /></td>
		
				<td width="25%" colspan="1" class="LABEL">Current Balance</td>
				<td width="25%" colspan="1" class="CONTROL"><bean:write
					name="issueOrgBean" property="strLFBalanceAmount" filter="false" /></td>
			</tr>
			<tr>
		
				<td width="25%" colspan="1" class="LABEL">LF Account Status</td>
				<td width="25%" colspan="3" class="CONTROL"><bean:write
					name="issueOrgBean" property="strLFAccountStatus" filter="false" /></td>
			</tr>
		</table>
	</div>
	...
	<div id="allDivId" style="display: block;">
		...
		<table class="TABLEWIDTH" align="center" cellspacing="1px">
			<tr>
				<bean:write name="issueOrgBean" property="strPatientDetails"
					filter="false" />
			</tr>
		</table>
		...
		<table class="TABLEWIDTH" align="center" cellspacing="1px">
			<tr>
				<bean:write name="issueOrgBean" property="strPatientDiagDetails"
					filter="false" />
			</tr>
		</table>--%>
	</div> 
	...
	<div style="display: none;">
		<div id="patientDetailsDivId" style="display: none;">
			<table class="TABLEWIDTH" align="center" cellspacing="1px">
				<tr>
					<bean:write name="issueOrgBean" property="strPatientDetails" filter="false" />
				</tr>
			</table>
			...
			<div id="patientDiagDetailsDivId" style="display: block;">
				<table class="TABLEWIDTH" align="center" cellspacing="1px">
					<tr>
						<bean:write name="issueOrgBean" property="strPatientDiagDetails" filter="false" />
					</tr>
				</table>
			</div>
		</div>
	...
	<input type="hidden" name="strCrNo" value="${issueOrgBean.strCrNo}" />
		
	<input type="hidden" name="crNo" value="${issueOrgBean.crNo}" />
	<input type="hidden" name="strId" value="${issueOrgBean.strId}" />
	<input type="hidden" name="itemCategory" value="${issueOrgBean.itemCategory}" />
	<input type="hidden" name="strItemCat" value="${issueOrgBean.itemCategory}" />
	
	<input type="hidden" name="strDuplicateItem" value="" />
		
	<input type="hidden" name="strConfCatCode"	value="${issueOrgBean.strConfCatCode}" />
	<input type="hidden" name="strIssueNo" value="${issueOrgBean.strIssueNo}" />
	<input type="hidden" name="strIssueDtl"	value="${issueOrgBean.strIssueDtl}" />
	<input type="hidden" name="disFlag" value="${issueOrgBean.disFlag}" />
	<input type="hidden" name="mode" value="${issueOrgBean.strMode}" />
	...
	<input type="hidden" name="strCtDate" value="${issueOrgBean.strCtDate}" />
	...
	<input type="hidden" name="strErrMsg" value="${issueOrgBean.strErrMsg}" />
	<input type="hidden" name="strBillingHiddenValue" value="${issueOrgBean.strBillingHiddenValue}" />
	<input type="hidden" name="strTariff_Flag" value="1" />
	<input type="hidden" name="strPatStatus" value="${issueOrgBean.strPatStatus}" />
	<input type="hidden" name="strRowCount" value="${issueOrgBean.strRowCount}" />
						...
						<div class="modal-body" id="modalSpaceId">
							<div  id='payDtlCDMenuInner ' style="display: block">
								...
								<div class="row rowFlex reFlex">
									...
									<div class="col-sm-6"><input type="text" class="form-control" tabindex="1" maxlength="11" placeholder="(DD-Mon-YYYY)" name="strAuthDate" value="${issueOrgBean.strCtDate }"  style="border:0.5px solid black;"/></div>
								</div>
								<div class="row rowFlex reFlex">
								...
								<div class="row rowFlex reFlex">
									...
									<div class="col-sm-6">
										<input type="text" tabindex="1" placeholder="(DD-Mon-YYYY)" class="form-control"
											...
											value="${issueOrgBean.strCtDate }" style="border:0.5px solid black;"/>
									</div>
								</div>
							</div>
						</div>
...
<jsp:include page="issue_org_trans_search_rowNew.jsp"></jsp:include>

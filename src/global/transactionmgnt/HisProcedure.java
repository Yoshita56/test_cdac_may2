...
public final class HisProcedure {
	...
	/** The proc_single_value. */
	private List<String> proc_single_value = null;

	/** The proc_multiple_value. */
	...
	/** The proc_out_value. */
	Map<String, Object> proc_out_value = null; // used to store the value
	...
	/**
	 * flag indicating whether err parameter is defined in procedure or not. It
	...
	public void setInValue(String parameterName, String val) {
		...
		if (this.proc_single_value == null)
			this.proc_single_value = new ArrayList<String>(2);
	 ...
	 *            - Procedure Parameter Name.
	 * @param outputType
	 *            <b>1</b>-String and <b>2</b>-WebRowSet.
	 */
	public void setOutValue(String parameterName, int outputType) {
		...
		if (this.proc_out_index == null)
			this.proc_out_index = new ArrayList<String>(1);

		// ERR PARAMETER MUST EXIST IN THE PROCEDURE
		if (parameterName.toUpperCase().trim().equals("ERR"))
			errFlag = true;

		this.proc_single_value.add(parameterName);
		this.proc_single_value
				.add("OUTPARAMETER#" + String.valueOf(outputType));

		index = this.proc_single_value.size() / 2;// change forpostgres
		this.proc_out_index.add(parameterName + "#"
				+ String.valueOf(outputType) + "#" + index);
	}
	...
	public Object getOutputValue(String parameterName) throws Exception {
		...
		try {
			...
			this.errMsg = "HisProcedure.getOutputValue(Procedure Name : "
					+ this.proc + ") -->\n" + e.getMessage();
			throw new Exception(this.errMsg);
		}
	 ...
	 *            - Connection Object.
	 * @throws Exception
	 *             - if any Exception arrives at the time of execution.
	 */
	public void executeProc(Connection conn) throws Exception {
		...
		try {
			if (!this.proc.equals("")) {
				...
				if (this.proc_single_value != null) {
					...
					for (int i = 0; i < lcl_col_size; i++) {
						...
						keyValue = ((String) this.proc_single_value
								.get((2 * i) + 1)).split("#");
						...
						if (keyValue[0].equals("OUTPARAMETER")) {
							...
							/*
							 * oracle if(keyValue[1].equals("1")) //string
							 * cs.registerOutParameter
							 * (keyName,java.sql.Types.VARCHAR); else //web row
							 * set
							...
							strProcValue = (String) this.proc_single_value
									.get((2 * i) + 1);
							// oracle
							// cs.setString(keyName,strProcValue);

							// postgres
							if (strProcValue == null
									|| strProcValue.trim().equals(""))
					...
					for (int i = 0; i < lcl_col_size; i++) {
						...
						if (keyValue[1].equals("1")) { // string return value
							...
							if (keyValue[0].toUpperCase().trim().equals("ERR")) {
								...
								if (!strTemp.equals("")
										|| strTemp.trim().length() > 0) {
									this.errMsg = "Error thrown by procedure : "
											+ strTemp;
									throw new Exception(this.errMsg);
								}
							}
							...
							// postgres
							rs = (ResultSet) cs.getObject(Integer
									.parseInt(keyValue[2]));
			...
			this.errMsg = "HisProcedure.executeProc(" + strProcArr[0] + "("
					+ this.procErrStr + ")) -->\n" + e.getMessage();
			throw new Exception(this.errMsg);
		} finally {
			try {
	...
	/**
	 * used to add procedure into batch whether it is parameterized procedure or
	 * not. It does not execute the procedure.
	 * 
	 * @throws Exception
	 *             - Either Mismatch in previous column size & current size
	 */
	public void execute() throws Exception {
		...
		try {
			if (this.proc_single_value != null)
				valueSize = this.proc_single_value.size() / 2;
			if (this.colSize == -1)
			...
			if (this.colSize == valueSize) {
				...
				if (this.proc_single_value != null) {
					if (this.proc_multiple_value == null)
						this.proc_multiple_value = new ArrayList<Object[]>(1);
	...
	/**
	 * This function executes a single procedure at a time which is in batch. It
	 * is called from {@link HisDAO#fire()} method. This method does not set the
	 * output parameter.
	 * 
	 * @param conn
	 *            - Connection Object
	...
	public void executeProc_Batch(Connection conn) throws Exception {
		...
		try {
			...
			if (!this.proc.equals("")) {
				if (this.cs == null)
					this.cs = conn.prepareCall(this.proc);

				if (this.proc_multiple_value != null) {
					tempList = (Object[]) this.proc_multiple_value
							.get(this.gblIndex++);
			...
			strProcArr = (this.proc.replace("(", "#")).split("#");
			this.errMsg = "HisProcedure.executeProc_Batch(" + strProcArr[0]
					+ "(" + this.procErrStr + ")) -->\n" + e.getMessage();
			throw new Exception(this.errMsg);
		} finally {
			tempList = null;

 ...
 */
public class CashCollectionOnlineTransDATA {
	...
	public static boolean init(HttpServletRequest request,CashCollectionOnlineTransFB formBean)
	{
		...
		try
		{
			...
			CashCollectionOnlineTransDATA.initOnLineDetails(formBean, bo, voObj);
		...
		catch (Exception e)
		{
			...
			{
				HisException eObj = new HisException("Billing","CashCollectionTransDATA->init()", msgStr);
				formBean.setStrErrMsg("Application Error [ERROR ID : "+ eObj.getErrorID()+ "], Contact System Administrator! ");
				formBean.setStrCrNo("");
				eObj = null;
			}
	...
	{		
		...
		{						
			...
			String strOnLineView = CashCollectionOnlineTransHLP.getOnLineDetailsView(voObj);
	...
	public static void getOnlineDtls(HttpServletRequest request,HttpServletResponse response) 
	{
		...
		try 
		{
			...
			if (!voObj.getStrBillingService().equals("21")) //Not Final Bill
			{
				...
				{
					clientView.append(CashCollectionOnlineTransHLP.getOnLineTariffDetailsView(voObj));
				}
				...
				if (voObj.getStrRequestType().equals("2")&& (voObj.getStrBillingService().equals("10")|| 
					voObj.getStrBillingService().equals("11") || voObj.getStrBillingService().equals("12")|| voObj.getStrBillingService().equals("25"))) 
				{
					clientView.append(CashCollectionOnlineTransHLP.getOnLineRefundTariffDetailsView(voObj));
				}
		...
		catch (Exception e) 
		{
			...
			try 
			{
				HisException eObj = new HisException("Billing","CashCollectionTransDATA->getOnlineDtls()", strmsgText);
	...
	public static boolean insertOnlineReceiptPartPayment(CashCollectionOnlineTransFB formBean , HttpServletRequest request) 
	{
		...
		{
			bo = new CashCollectionOnlineTransBO();
			hisUtil = new HisUtil("Billing", "CashCollectionOnlineTransDATA");
			...
			if (!voObj.getStrMsgType().equals("1")) 
			{
				String smsMsg="Dear "+formBean.getStrPatientDtlHidVal().replace("^","#").split("#")[7]  +", Payment of Rs. "+formBean.getStrOnlineTotalRecAmount()+" Received Against Bill No. "+formBean.getStrBillNo()+ "/1 on "+hisUtil.getDSDate("")+" "+hisUtil.getDSDate("hh:mi:ss:AM")+". AIIMS Mangalagiri";
		...
		catch (Exception e) 
		{
			...
			HisException eObj = new HisException("Billing Transaction","CashCollectionTransDATA.insertOnlineReceiptPartPayment()-->",err);
			formBean.setStrErrMsg("Application Error [ERROR ID : "+ eObj.getErrorID() + "],Contact System Administrator! ");
			eObj = null;
		} 
		finally 
	...
	public static boolean insertOnlineReceiptServices(CashCollectionOnlineTransFB formBean  , HttpServletRequest request) 
	{
		...
		{
			e.printStackTrace();
			HisException eObj = new HisException("Billing Transaction","CashCollectionTransDATA.insertOnlineReceiptServices()-->",e.getMessage());
	...
	public static boolean insertOnlineRefundServices(CashCollectionOnlineTransFB formBean  , HttpServletRequest request) 
	{
		...
		try 
		{
			 ...
			 * (voObj.getStrOnlinePaymentMode()[0].equals("7"))//Payment Mode=Wallet {
			 * String
			 * smsMsg="You Wallet Is Credited With Rs. "+voObj.getStrOnlineTotalRecAmount()
			 ...
			 * getStrAvlWalletMoney())+Double.parseDouble(voObj.getStrOnlineTotalRecAmount()
			 * ));
			 * sms.sendTextSMSThroughNICSMSGateway("","","","",formBean.getStrMobileNo(),
			 * smsMsg); } }
			 */
			...
			if (!voObj.getStrMsgType().equals("1")) 
			{
				...
				else if( !voObj.getStrWalletNo().equals("0") && !voObj.getStrWalletNo().equals("")) 
				{
					if(!voObj.getStrWalletNo().equals("0"))
						formBean.setStrAccountNo(voObj.getStrWalletNo());
					

						
					PrintHLP.IPD_REFUND_WALLET(formBean.getStrBillNo(), formBean.getStrRefundReceiptNo(), formBean.getStrAccountNo(),
							formBean.getStrBillingService(), formBean.getStrHospitalCode() , request , 0);
					String fileName = HisUtil.getParameterFromHisPathXML("TEMP_PATH")+ "/"+ "Billing"+ request.getSession().getAttribute("SEATID").toString() + ".dat";
					formBean.setFilePath(fileName);
					request.setAttribute("filePath", fileName);
		...
		{
			HisException eObj = new HisException("Billing Transaction","CashCollectionTransDATA.insertOnlineRefundServices()-->",e.getMessage());
			formBean.setStrErrMsg("Application Error [ERROR ID : "+ eObj.getErrorID() + "],Contact System Administrator! ");
			eObj = null;
		} 
		finally 
	...
	public static boolean insertOnlineFinalAdjustment(CashCollectionOnlineTransFB formBean  , HttpServletRequest request) 
	{
		...
		{
			HisException eObj = new HisException("Billing Transaction","CashCollectionTransDATA.insertOnlineFinalAdjustment()-->",e.getMessage());
			formBean.setStrErrMsg("Application Error [ERROR ID : "+ eObj.getErrorID() + "],Contact System Administrator! ");
			eObj = null;
		}
		return fRes;

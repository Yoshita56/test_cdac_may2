...
public class IndentApprovalDeskDAO 
{
	...
	public static void getAppLevelBasedOnSeatId(IndentApprovalDeskVO vo) 
	{
		// Declearing Variable
		String retVal = null;
		int funcIndex = 0;
		...
		try {
		    ...
		    */
			...
			vo
					.setStrMsgString("IndentApprovalDeskDAO.getAppLevelBasedOnSeatId() --> "
							+ e.getMessage());
			vo.setStrMsgType("1");
		}
	}
	...
	public static void getItemDetails(IndentApprovalDeskVO vo) 
	{
    	...
    	try 
		{
    		System.out.println("-----(IndentApprovalDeskDAO.getItemDetails)-PKG_MMS_VIEW.PROC_GET_INDENTITEM_DTL--- [ Mode- 1] -------");
	...
	public static void getIndentDetails(IndentApprovalDeskVO vo) 
	{
    	...
    	try 
		{
			...
			{	
    	    	System.out.println("--A---(IndentApprovalDeskDAO.getIndentDetails)-PKG_MMS_VIEW.Get_Indent_Details--- [ Mode- 1] -------");
			...
			{
				System.out.println("--B---(IndentApprovalDeskDAO.getIndentDetails)-PKG_MMS_VIEW.Get_Indent_Details--- [ Mode- 1] -------");
	...
	public static void getItemDetailsView(IndentApprovalDeskVO vo) 
	{
    	...
    	try 
		{
    		System.out.println("----- IndentApprovalDeskDAO . getItemDetailsView--PKG_MMS_VIEW.PROC_GET_INDENTITEM_DTL--- [ Mode- 1] -------");
	...
	public static void getLast_Issue_Qty_Of_Item_To_Indenter(IndentApprovalDeskVO vo,String strItemBrandId)
	{
		...
		try
		{
			...
			System.out.println(" ----------pkg_mms_view.New_Report_For_Acknowledge_Transfer_Issue ---[ Mode - 4 ]--- " );
			/*
			...
			*/
			...
			daoObj.setProcInValue(nProcIndex, "hosp_code", 		vo.getStrHospitalCode(),2);	    
			daoObj.setProcInValue(nProcIndex, "strid", 			vo.getStrToStoreId(),3);			
			daoObj.setProcInValue(nProcIndex, "itemcategno", 	vo.getStrItemCatgNo(),4);     
			daoObj.setProcInValue(nProcIndex, "InstituteId", 	vo.getStrStoreId(),5);	     
			daoObj.setProcInValue(nProcIndex, "start_date", 	vo.getStrFinancialStartYear(),6);  
			daoObj.setProcInValue(nProcIndex, "end_date", 		vo.getStrFinancialEndYear(),7);    
			daoObj.setProcInValue(nProcIndex, "SupplierId", 	"0",8);			 
			daoObj.setProcInValue(nProcIndex, "StrlpItemId", 	strItemBrandId,9);		     
			...
			if(strErr.equals(""))
			{
				vo.setLastIssueQtyWs(ws);
			}
		}
		catch(Exception _err)
		{
			vo.setStrMsgString("IndentApprovalDeskDAO.getLast_Issue_Qty_Of_Item_To_Indenter() --> "	+ _err.getMessage());
			vo.setStrMsgType("1");
		}
		 finally {
				if (daoObj != null) {
	...
	public static void getIndentDetailsView(IndentApprovalDeskVO vo) 
	{
    	...
    	try 
		{
    		System.out.println("----- IndentApprovalDeskDAO . getIndentDetailsView--PKG_MMS_VIEW.Get_Indent_Details--- [ Mode- 1] -------");
	...
	public synchronized static void INSERT(IndentApprovalDeskVO vo) 
	{
    	...
    	try 
		{
    		...
    		System.out.println("-------------- IndentApprovalDeskDAO . INSERT ------PKG_MMS_DML . DML_APPROVALITEM_DTL [ Mode - 1] ---[ START ]--------");
    		...
    		dao = new HisDAO("MMS","transactions.Indent ApprovalDeskDAO.INSERT()");
    		    ...
    		    System.out.println("-------------- PKG_MMS_DML.DML_APPROVAL_DTL [ Mode - 1 ] -----------START----------");
    			System.out.println("Req No							-->>"+vo.getStrReqNo());
				System.out.println("Req Typ Id						-->>"+vo.getStrReqTypeId());
                System.out.println("Store Id						-->>"+vo.getStrStoreId());
                System.out.println("Level tyep						-->>"+vo.getStrLevelType());
                System.out.println("App_Type-[1-Approved,2-Rejected ]->>"+vo.getStrApprovalType());
//    		   //System.out.println("to Store Id-->>"+vo.getStrToStoreId());
//    		   //System.out.println("Remarks-->>"+vo.getStrRemarks());
                
               /*
                exec PKG_MMS_DML.DML_APPROVAL_DTL(modval=>'1',
                		strId=>'10201236',reqNo=>'170221223151841',
                		hosp_code=>'37913',levelType=>'1',approvalType=>'2',
                		ipAddress=>'0.0.0.0',finStartDate=>'01-Apr-2014',finEndDate=>'31-Mar-2015',
                		remarks=>'Rejected By User',seatId=>'12359',deliveryDate=>'',
                		returnType=>'',commTypeId=>'0',commRemarks=>'',
                		itemDetail=>':X1',appNo=>':X2',
                		status=>':X3',
                		lstStatus=>':X4',err=>':X5')
               	*/ 		    
			    ...
			    procIndex1 = globalDao.insert(dao);  //Procedure Name PKG_MMS_DML.DML_APPROVAL_DTL [ Mode - 1 ]
									
	         
			    dao.fire();     // Here we Execute the Procedure
			    
			    System.out.println("-------------- PKG_MMS_DML.DML_APPROVAL_DTL [ Mode - 1 ] ----------- END ----------");
		        ...
		        System.out.println("-----------------------------------");    		
			      ...
			      {	  
				    INSERTINTABLE(vo,itemDetail,appNo,status,lstStatus);   // PKG_MMS_DML . DML_APPROVALITEM_DTL [ Mode - 1]
			      }
			      ...
			      System.out.println("-------------- IndentApprovalDeskDAO . INSERT ---------[ END ]--------");	  
	     	}
			catch (Exception e) 
    	    {
    		e.printStackTrace();
	...
	public static void INSERTINTABLE(IndentApprovalDeskVO vo,String itemDetail,String appNo,String status,String lstStatus) 
	{
		...
		try 
		{
			...
			*/
    		...
    		System.out.println("---------------- IndentApprovalDeskDAO . INSERTINTABLE---------[ START ]-----PKG_MMS_DML . DML_APPROVALITEM_DTL [ Mode - 1]------");
 			...
 			for(int i = 0;i<length;i++)
			{
 				temp = vo.getStrInsertHiddenValue()[i].split("\\@");
				 	...
				 	if(vo.getStrReqTypeId().equals("16")||vo.getStrReqTypeId().equals("19")||vo.getStrReqTypeId().equals("47")||vo.getStrReqTypeId().equals("18")||vo.getStrReqTypeId().equals("65"))
					{
				 	...
				 	System.out.println("lstStatus--"+i+"->>>"+lstStatus);					 	
				 	System.out.println("appNo--"+i+"->>>"+appNo);	
				 	System.out.println("------------------------------------");
 			  ...
 			  System.out.println("---------------- IndentApprovalDeskDAO . INSERTINTABLE-------- END ------------");
		      synchronized(dao)   
			  {
	...
	public synchronized static void INSERT_SPECIAL_APPROVAL(IndentApprovalDeskVO vo) 
	{
		HisDAO       dao  = null;
		int    procIndex1 = 0;
		String itemDetail = "";
...
//    		   //System.out.println("Retunr Type-->>>"+vo.getStrReturnType());
				...
				globalDao.setReturnType(vo.getStrReturnType());
				globalDao.setCommRemarks(vo.getStrCommitteRemarks());
			    procIndex1 = globalDao.insert_Special(dao);  //Procedure Name DML_APPROVAL_DTL [ Mode - 2]
    	    ...
    	    {
    		e.printStackTrace();
			vo.setStrMsgString("-->IndentApprovalDeskDAO.INSERT_SPECIAL_APPROVAL()-->"
					+ e.getMessage());
			vo.setStrMsgType("1");
		} 
    	finally 
	...
	public static void INSERTINTABLE_SPECIAL_APPROVAL(IndentApprovalDeskVO vo,String itemDetail,String appNo,String status,String lstStatus) 
	{
		HisDAO   dao         = null;
		HisUtil  util        = null;
		String[] temp        = null;
		...
		try 
		{
			            ...
			            util = new HisUtil("MMS","transactions.IndentApprovalDeskDAO.INSERTINTABLE_SPECIAL_APPROVAL()");
			       strCtDate = util.getASDate("dd-MMM-yyyy");
			        tableDao = new DmlApprovalItemDtlDAO();
    		             dao = new HisDAO("MMS","transactions.IndentApprovalDeskDAO.INSERTINTABLE_SPECIAL_APPROVAL()");
	...
	public static void getStockTableHLP(IndentApprovalDeskVO vo) 
	{
		WebRowSet  wb = null;
		HisDAO daoObj = null;
		int nProcIndex = 0;
		...
		{    	    		    	
    		...
    		System.out.println("-----(IndentApprovalDeskDAO.getStockTableHLP)-PKG_MMS_VIEW2.proc_stock_batch_table--- [ Mode- 1] -------");    		
    		String strProcName = "{call PKG_MMS_VIEW2.proc_stock_batch_table(?,?,?,?,?  ,?)}"; // 6
			daoObj = new HisDAO("MMS Transactions","IndentApprovalDeskDAO");			
			nProcIndex = daoObj.setProcedure(strProcName);
     		daoObj.setProcInValue(nProcIndex, "modval",   	"1",1);     		
     		daoObj.setProcInValue(nProcIndex, "hosp_code",	vo.getStrHospitalCode(),2);     		
	     	daoObj.setProcInValue(nProcIndex, "seat_id",  	vo.getStrSeatId(),3);			
			daoObj.setProcInValue(nProcIndex, "pkey",     	vo.getStrItemId()+"$"+vo.getStrItemBrandId()+"$"+vo.getStrStoreId()+"$"+vo.getStrTostoreId()+"$"+vo.getStrLastAppLevel(),4);			
			daoObj.setProcOutValue(nProcIndex,"err", 		1,5);
			daoObj.setProcOutValue(nProcIndex,"resultset", 	2,6);
			...
			if (strErr.equals("")) 
			{
				...
				while(wb.next())
				{
					System.out.println("TABLE FROM DB"+wb.getString(1));
					vo.setStrDBTabel(wb.getString(1));
				}

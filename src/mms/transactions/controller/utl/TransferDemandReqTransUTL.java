...
/**
 * The Class TransferDemandReqTransUTL.
 */
public class TransferDemandReqTransUTL extends TransInterface {
	...
	public String getMasterName() {
		...
		masterName = "Transfer Request For Shortage";  
		// }
	...
	public String getMainQuery(String cmb[]) {
		...
		brMainQuery.append("SELECT C.HSTNUM_REQUEST_NO||'@'||C.HSTNUM_SLNO ||'@'|| C.MODIFY_CANCEL_FLAG ||'^'|| C.HSTNUM_REQUEST_NO||'^'||");
		brMainQuery.append("C.REQ_DATE|| '^'||C.ITEM_NAME || '^'||");
		brMainQuery.append("C.REQ_SEC_QTY || ' ' || C.UNIT_NAME || '^'||");
		brMainQuery.append("C.TRANSFER_ACK_QTY || ' ' || C.UNIT_NAME");
		if (strStatus.equals("0")) {
			brMainQuery.append("|| '^'|| C.STS");
		}
		brMainQuery.append(" || '^' || MODIFY_CANCEL_FLAG  FROM ( "
				+ "SELECT HSTNUM_REQUEST_NO,HSTNUM_SLNO,TO_CHAR(HSTDT_REQUEST_DATE,'DD-Mon-YYYY') AS REQ_DATE,"
				+ "MMS_MST.GET_ITEM_DTL(1,GNUM_HOSPITAL_CODE,HSTNUM_ITEMBRAND_ID) AS ITEM_NAME,"
				+ "NVL(HSTNUM_REQ_QTY,0) || '/' || NVL(HSTNUM_SANCTION_QTY,0) AS REQ_SEC_QTY,"
				+ "NVL(HSTNUM_TRANSFER_QTY,0) || '/' || (NVL(HSTNUM_ACK_QTY,0) + NVL(HSTNUM_BREKAGE_QTY,0)) AS TRANSFER_ACK_QTY,"
				+ "DECODE(GNUM_STATUS,0,1,DECODE(GNUM_STATUS,40,DECODE(HSTDT_LST_APPROVAL_DATE,NULL,1,0),0)) AS MODIFY_CANCEL_FLAG,"
				+ "MMS_MST.GETUNITNAME(GNUM_HOSPITAL_CODE,HSTNUM_REQ_QTY_UNITID) AS UNIT_NAME, " + "CASE "
				+ "WHEN (GNUM_STATUS >= 0 AND GNUM_STATUS < 40) THEN 'Approval In-Process' " + "WHEN (GNUM_STATUS = 40) THEN 'Order In-Process' "
				+ "WHEN (GNUM_STATUS = 45 AND HSTNUM_TOT_PENDING_ACK > 0) THEN 'Ack In-Process' "
				+ "WHEN (GNUM_STATUS = 45 AND HSTNUM_TOT_PENDING_ORDER > 0) THEN 'Transfer In-Process' " + "ELSE '--' " + "END AS STS , gnum_isvalid AS is_valid "
				+ "FROM HSTT_TRANSFER_SHORT_REQ_DTL " + "WHERE GNUM_ISVALID = 1 " + "AND GNUM_HOSPITAL_CODE = " + hospCode + " ");
		if (strStatus.equals("0")) {
			...
			if (strStatus.equals("1")) {
				...
				if (strStatus.equals("2")) {
					brMainQuery.append("AND GNUM_STATUS = 40 ");
				} else {
					// TRANSFER IN-PROCESS
					if (strStatus.equals("3")) {
						...
						if (strStatus.equals("4")) {
							brMainQuery.append(" AND GNUM_STATUS = 45 AND HSTNUM_TOT_PENDING_ACK > 0 ");
						} else {
							// REJECTED [LAST 3 MONTHS]
							if (strStatus.equals("5")) {
								brMainQuery
										.append(" AND GNUM_STATUS = 99 AND HSTDT_LST_APPROVAL_DATE >= TRUNC(SYSDATE) - 90 AND HSTDT_LST_APPROVAL_DATE < TRUNC(SYSDATE) + 1");
							} else {
								// CLOSED [LAST 3 MONTHS]
								brMainQuery
										.append(" AND GNUM_STATUS = 50 AND HSTDT_LST_APPROVAL_DATE >= TRUNC(SYSDATE) - 90 AND HSTDT_LST_APPROVAL_DATE < TRUNC(SYSDATE) + 1");
							}
						}
					}
				}
	...
	public String[] getColumnHeader() {
		...
		if (cmbVal != null && cmbVal[1].equals("0")) 
		{
			...
			strColumnHeader[3] = "Req/Sanc_Quantity";
			strColumnHeader[4] = "Transfer/Ack_Qty";
	...
	public String[] getComboQuery() {
		...
		comboQuery[0] =
				"SELECT HSTNUM_STORE_ID, hststr_store_name FROM hstt_store_mst a  WHERE gnum_isvalid = 1"
				//+ " AND SSTNUM_DWH_TYPE_ID IN (10,11,13,14,15,12,16,17,20,21,19,24 ) "
		...
		comboQuery[1] = "0^All#1^Approval In-Process#2^Order In-Porcess#3^Transfer In-Process#4^Ack In-Process#5^Rejected#6^Closed";
		return comboQuery;
	}
	...
	public String getJsFiles() {
		String files = "../../mms/js/transferDesk_trans.js";
		return files;

	...
	public String getMainQuery(String cmb[]) {
	    ...
	    if(cmb != null)
	    {
			...
			//	brMainQuery.append("MMS_MST.GET_STORE_DTL(1,A.GNUM_HOSPITAL_CODE,HSTNUM_TOSTORE_ID) || '^' || decode(HSTNUM_INDENT_STATUS,0,'Pending',1,'App-In Proceess',10,'I Approval',11,'II Approval',45,'Indent Closed',20,'Partial Approved',40,'Final Approved',49,'In Process',99,'External / NA',111,'Items Received',50,'Processed',53,'Processed',HSTNUM_INDENT_STATUS)  || '^'||");
				...
				brMainQuery.append(" AND HSTDT_REQ_DATE > TRUNC(SYSDATE) - 180 ");
		    ...
		    else
		    {
		    	...
		    	brMainQuery.append("MMS_MST.GET_STORE_DTL(1,A.GNUM_HOSPITAL_CODE,HSTNUM_TOSTORE_ID) ");
		 		...
		 		brMainQuery.append( httpSession.getAttribute("HOSPITAL_CODE").toString());
		 		brMainQuery.append(" AND HSTDT_REQ_DATE > TRUNC(SYSDATE) - 180 ");
		 		brMainQuery.append(" AND A.HSTNUM_STORE_ID in ( SELECT GNUM_COLUMN_VALUE  FROM GBLT_ROLE_SEAT_TABLE_DTL P  WHERE P.gnum_metatable_id = 117 AND P.gnum_hospital_code = A.gnum_hospital_code AND P.gnum_seatid =(select Pkg_Usermgmt.fun_getseatid("+httpSession.getAttribute("SEATID").toString()+", A.GNUM_HOSPITAL_CODE) from dual )) ");
		 		brMainQuery.append(" AND  SSTNUM_REQTYPE_ID NOT IN(80,81,82,83,84,85)");
		    }
...
//			else
			...
			{
			   brMainQuery.append(" AND (HSTNUM_INDENT_STATUS > 0 AND HSTNUM_INDENT_STATUS < 50) ");
			}
//			//final approved
...
//			}	
		...
		System.out.println("Demand desk Main Query-[IndentTransUTL]->>>>"+brMainQuery.toString());
	...
	public String[] getComboQuery()
	{
       ...
       // comboQuery[1] = comboQuery[1].concat(mms.qryHandler_mms.getQuery(1,"select.StoreItemCategory.cond.0")).replace("?",hosCode);
        ...
        //select.StoreItemCategory.cond.0 = AND GNUM_HOSPITAL_CODE=? and SSTNUM_ITEM_CAT_NO IN (select SSTNUM_ITEM_CAT_NO from sstt_item_category_mst where gnum_hospital_code = 100 and gnum_isvalid=1) ORDER BY CAT_NAME
	...
	{
		String[] status = { "Urgent", "7", "CYAN", "Urgent/External Request"};
		return status;
	...
	public String[] getUserDefinedButtons() 
	{
			    ...
			    // strButtons = new String[6];
				 ...
				 strButtons[2] = "View@buttonLogicsOnClick2(4,'VIEW','View')@1@007bb6@glyphicon-fullscreen";
	...
	public String[] getOrderBy() {
		String orderBy[] = { "2", "TO_DATE(A.HSTDT_REQ_DATE)" };
		return orderBy;
	}
	@Override

...
public class DrugInventoryWithProgramTransUTL extends TransInterface {
		...
		mms.qryHandler_mms.res = res;*/
			...
			if (cmb != null) {
				...
				if (cmb[3].equals("16")) {
					...
					brMainQuery
							.append(" SELECT    c.hstnum_store_id|| '@'|| c.hstnum_item_id|| '@'|| c.hstnum_itembrand_id|| '@'|| split_part(c.hststr_batch_no,'~',1)");
					brMainQuery
							.append(" || '@'|| c.gnum_hospital_code|| '@'|| hstnum_stock_status_code  || '^'|| c.drug_code || '^'|| c.item_brand_name|| '^'|| split_part(c.hststr_batch_no,'~',1) || '^' ||  C.HSTNUM_PO_NO  || '^' ||  MMS_MST.GET_PROGRAMME_NAME(1,c.gnum_hospital_code,C.hstnum_programme_id)");
					brMainQuery
					...
					brMainQuery
							.append("|| '^'|| mms_mst.get_reorder_flag (c.gnum_hospital_code,c.hstnum_itembrand_id,c.hstnum_item_id,c.hstnum_store_id,c.hstnum_inhand_qty,split_part(c.hststr_batch_no,'~',1),1)");
					brMainQuery
	...
	public String[] getComboQuery() {
		...
		comboQuery[0] = "select HSTNUM_STORE_ID||'^'||HSTNUM_PHYSTOCK_FLAG,HSTSTR_STORE_NAME from HSTT_STORE_MST A where  GNUM_ISVALID = 1  AND  GNUM_HOSPITAL_CODE ="
				...
				+ " AND UPPER(q.gstr_table_name) = UPPER('HSTT_STORE_MST')"
				+ " AND UPPER(q.gstr_column_name) = UPPER('HSTNUM_STORE_ID')"

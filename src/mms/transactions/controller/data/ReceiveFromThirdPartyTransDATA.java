...
public class ReceiveFromThirdPartyTransDATA {
	...
	public static void insert(ReceiveFromThirdPartyTransFB formBean) {
		...
		try {
			...
			vo = (ReceiveFromThirdPartyTransVO) TransferObjectFactory.populateData("mms.transactions.vo.ReceiveFromThirdPartyTransVO",formBean);
...
//			vo.setStrFinancialEndYear(mmsUtil.getStrFinancialEndDate(vo.getStrStoreId() , vo.getStrHospitalCode()));
			...
			String strTemp = ReceiveFromThirdPartyTransHLP.getReceivedItemsList(formBean,vo);
	...
	public static void printVoucherDtl(ReceiveFromThirdPartyTransFB fb, HttpServletRequest request, HttpServletResponse response) {
		...
		ReceiveFromThirdPartyTransVO vo    = null;			
			...
			try 
			{
				   System.out.println(" ------- ReceiveFromThirdPartyTransDATA.printVoucherDtl  ---- pkg_mms_view.proc_receivedItem_dtl_individual [ Mode - 1 ]--- ");
				 
					util = new HisUtil("MMS Transactions", "ReceiveFromThirdPartyTransDATA");
				  ...
				  // bo.getImgHeader(vo);
				   ...
				   strResult=ReceiveFromThirdPartyTransHLP.getvoucherPrintDetails(fb,vo);

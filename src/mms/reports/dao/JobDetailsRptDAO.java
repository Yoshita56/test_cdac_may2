...
public class JobDetailsRptDAO {
	
	
	public static void getViewDtl(JobDetailsRptVO vo)
	{
		...
		try
		{
			daoObj  = new HisDAO("MMS","JobDetailsRptDAO");
			System.out.println(" ----------> getStrSeatId()------------ " +  vo.getStrSeatId());
			System.out.println(" ----------> getStrHospitalCode() ---- " +  vo.getStrHospitalCode());
			System.out.println(" ----------> getStrTypeId() ---- " +  vo.getStrTypeId());
			System.out.println(" ----------> getStrTypeName() ---- " +  vo.getStrTypeName());
			...
			daoObj.setProcInValue(nProcIndex, "catg_id",vo.getStrTypeId(),3);
			...
			daoObj.setProcInValue(nProcIndex, "seat_id",vo.getStrTypeId(),7);
		...
		{
			vo.setStrMsgString("JobDetailsRptDAO.getViewDtl() --> "	+ _err.getMessage());
			vo.setStrMsgType("1");
		}
		 finally {
				if (daoObj != null) {
	...
	public static void getImageHeader(JobDetailsRptVO vo)
	{
		String strFuncName = "";
		String strLogoName = "";
		...
		try 
		{
			dao = new HisDAO("mms","JobDetailsRptDAO");
			...
			e.printStackTrace();
			vo.setStrMsgType("1");
			vo.setStrMsgString("JobDetailsRptDAO.getImageHeader() --> "+ e.getMessage());

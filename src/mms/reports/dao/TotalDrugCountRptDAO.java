...
public class TotalDrugCountRptDAO {
	
	public static void getStoreList(TotalDrugCountRptVO voObj) {
		...
		try {
			...
			daoObj = new HisDAO("MMS Transactions","TotalDrugCountRptDAO");
		//	
			...
			voObj
					.setStrMsgString("TotalDrugCountRptDAO.getStoreList() --> "
							+ e.getMessage());
	 ...
	 * @param voObj
	 */
	public static void getDistrictStoreList(TotalDrugCountRptVO voObj) {
	...
	public static void getItemCatList(TotalDrugCountRptVO voObj) {
...
String strProcName = "{call Pkg_Mms_Rpt.Rptm_item_category_list(?,?,?,?,?)}";
		...
		try
		{
			daoObj=new HisDAO("Item Location","TotalDrugCountRptDAO");
			//
			nProcIndex = daoObj.setProcedure(strProcName);
			daoObj.setProcInValue(nProcIndex, "modeval", "1",1);
			...
			voObj
					.setStrMsgString("TotalDrugCountRptDAO.getItemCatList() --> "
							+ e.getMessage());
			voObj.setStrMsgType("1");

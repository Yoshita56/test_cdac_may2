...
public class DossierRptDAO {
	
	/**
	 * This function is used to fetch Store Combo Detail
	 * @param _DossierRptVO
	 */
	public static void setStoreCombo(DossierRptVO _DossierRptVO)
	{
		String strproc_name = "";
		HisDAO dao = null;
		int nprocIndex = 0;
		...
		try 
		{
				...
				dao.setProcInValue(nprocIndex, "hosp_code", _DossierRptVO.getStrHospCode(),2);
				...
				{	
				   if(wb.next())
				   {
					   _DossierRptVO.setStrStoreId(wb.getString(1));					   
					   wb.beforeFirst();
				   }
				}				
				if (strerr.equals("")) 
				{
					_DossierRptVO.setStrWS(wb);
				} 
				else 
				{
		...
		{
			_DossierRptVO.setStrMsgString("IssueDetailRptDAO.setStoreCombo() --> "+ e.getMessage());
			_DossierRptVO.setStrMsgType("1");
		} 
		finally 
		{
			if (dao != null) 
	 ...
	 * @param _DossierRptVO
	 */
	public static void setItemCategCombo(DossierRptVO _DossierRptVO)
	{
		String strproc_name = "";
		HisDAO dao = null;
		int nprocIndex = 0;
		...
		try 
		{
				...
				nprocIndex = dao.setProcedure(strproc_name);
				System.out.println("modeval"+_DossierRptVO.getStrMode());
				dao.setProcInValue(nprocIndex, "modeval",_DossierRptVO.getStrMode(),1);
				dao.setProcInValue(nprocIndex, "hosp_code", _DossierRptVO.getStrHospCode(),2);
				dao.setProcInValue(nprocIndex, "storeId",_DossierRptVO.getStrStoreId(),3);							
				...
				if (strerr.equals("")) 
				{
					_DossierRptVO.setItemCategWS(wb);
             	} 
				else 
				{
					throw new Exception(strerr);
		...
		{
			_DossierRptVO.setStrMsgString("IssueDetailRptDAO.setItemCategCombo() --> "+ e.getMessage());
			_DossierRptVO.setStrMsgType("1");
		} 
		finally 
		{
	...
	public static void getDrugNameCombo(DossierRptVO vo)
	{
	...
	public static void getExistingBatchList(DossierRptVO vo) 
	{
	...
	public static void getHospitalName(DossierRptVO voObj) 
	{
		HisDAO daoObj = null;
		WebRowSet ws = null;

...
public class IssueDetailRptDAO {
	...
	public static void getReportData(IssueDetailRptVO voObj) 
	{
		HisDAO daoObj = null;
		WebRowSet ws = null;

		String strProcName = "{call pkg_mms_rpt.proc_issue_dtl_report(?,?,?,?,?,  ?,?,?,?,?,   ?)}"; //11
		int nProcIndex = 0;	
		String strErr = "";
		...
		try 
		{
			...
			daoObj.setProcInValue(nProcIndex, "itemid",voObj.getStrItemId() ,4);
		...
		{
			voObj.setStrMsgString("IssueDetailRptDAO.getReportData() --> "+ e.getMessage());
			voObj.setStrMsgType("1");
		} 
		finally 
		{

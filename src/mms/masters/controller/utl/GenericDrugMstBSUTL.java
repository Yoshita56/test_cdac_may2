 ...
 */
public class GenericDrugMstBSUTL implements MasterInterface {
	...
	public String getButtons() {
		...
		br.append("<a style=cursor:pointer><img src='../../hisglobal/images/btn-view.png'	tabindex='0' title='View'	style='cursor: pointer;' onKeyPress='if(event.keyCode==13) genericDrugViewBS();'			onClick='genericDrugViewBS();'		></a>");
...
*/
		...
		br.append("<br><a href='#' class='btn btn-sm btn-primary' id='' onKeyPress='if(event.keyCode==13) addDrugMst();'				onClick='addDrugMst();'	'>Add</a>");
		br.append("<a href='#' class='btn btn-sm btn-primary' id='' onKeyPress='if(event.keyCode==13) ModifyDrugMst();'			onClick='ModifyDrugMst();' 	>Modify</a>");
		br.append("<a href='#' class='btn btn-sm btn-primary' id='' onKeyPress='if(event.keyCode==13) deleteRecords(\"DELETE\");'	onClick='deleteRecords(\"DELETE\");'>Delete</a>");
		br.append("<a href='#' class='btn btn-sm btn-primary' id=''  data-toggle='modal' data-target='#viewModal'  onKeyPress='if(event.keyCode==13) genericDrugViewBS();' onClick='genericDrugViewBS();'>View</a>");
		br.append("<a href='#' class='btn btn-sm btn-primary' id='' onKeyPress='if(event.keyCode==13) report(\"REPORT\");'			onClick='report(\"REPORT\");'>Report</a>");
		return br.toString();
	}
	...
	public String[] getColumnHeader() {
		String col_header[] = {  "Generic Drug Name",
				"Inventory Unit", "Effective From" };
		return col_header;
	}
	...
	public String[] getComboHeader() {
		String[] comboHeader = { "0", "Group Name", "0", "SubGroupName", "1",
				"Record Status" };
		return comboHeader;
	}
	...
	public String[] getComboQuery() {
		...
		comboQuery[0] = mms.qryHandler_mms.getQuery(1, "select.drug.1")
				.replace("?", hosCode);
		comboQuery[1] = mms.qryHandler_mms.getQuery(1, "select.drug.2")
				.replace("?", hosCode);
	...
	public String getMainQuery(String[] cmb) {
		...
		String hosCode = MmsConfigUtil.GLOBAL_HOSPITAL_CODE;
		brMainQuery.append(mms.qryHandler_mms.getQuery(1, "select.drug.0")
				.replace("?", hosCode));
	...
	public String getMasterName() {
		...
		String masterName = "Generic Drug  Master";
	...
	public String getViewQuery() {
		return mms.qryHandler_mms.getQuery(1, "select.drug.11");

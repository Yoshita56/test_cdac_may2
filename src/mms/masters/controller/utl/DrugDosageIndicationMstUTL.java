...
/**
 * The Class DrugDosageIndicationMstUTL.
 */
public class DrugDosageIndicationMstUTL implements MasterInterface {
	...
	public String getMasterName() {
		...
		String masterName = "Drug Dosege & Indication Master";
		return masterName;
	}
	...
	public String getMainQuery(String[] cmb) {
		...
		brMainQuery.append(
				mms.qryHandler_mms.getQuery(1, "select.drugDosIndicationMst.0")
						.replace("?", hosCode)).append(
				" AND "
						+ mms.qryHandler_mms.getQuery(1,
								"select.drugDosIndicationMst.cond.0").replace(
								"?", "1"));
		if (cmb != null) {
			if (!cmb[0].equals("0")) {
		 ...
		 //System.out.println("Final Qry is:::"+brMainQuery.toString());
	...
	public String[] getDeleteQuery() {
		...
		delQuery[0] = mms.qryHandler_mms.getQuery(1,
				"delete.drugDosIndicationMst.0").replace("?", seatId);

		delQuery[0] = delQuery[0].concat(" WHERE "
				+ mms.qryHandler_mms.getQuery(1,
						"delete.drugDosIndicationMst.cond.0"));
	...
	public String getButtons() {
		...
		br.append("<a style=cursor:pointer><img src='../../hisglobal/images/btn-view.png'	title='View'	tabindex='0' onKeyPress='if(event.keyCode==13) showViewDrugDose(document.forms[0]);'	onClick='showViewDrugDose(document.forms[0]);'></a>");
